            ...
            <?php           
				...
				if($num>0)
				{
					...
					foreach($allData as $eachVal)
					{
						...
						//echo $eachVal['multi_id']."||".$eachVal['identical_function']."<br>";
					if($eachVal['multi_id']!="" || $eachVal['identical_function']==1)
					{
						...
						if(($row1[$k]['multi_event_date']!=$row1[$k-1]['multi_event_date']) || ($row1[$k]['id']!=$row1[$k-1]['id'])){
							...
							$objmul_date->multi_event_datewise($eachVal['id'],$row1[$k]['multi_event_date'],$date2);
					...
					?>
						<div class="party_details_box">
							<div class="box1">
								...
								<p class="sub"><a style="color:#056A86" href="<?php echo $obj_base_path->base_path(); ?>/venue/<?php echo $obj_venue->f('venue_id');?>"><strong><?php if($_SESSION['langSessId']=='eng') { echo $obj_venue->f('venue_name'); } else { echo $obj_venue->f('venue_name_sp'); };?></strong></a></p>
							...
							<div class="box2">
							<div class="heading"><a href="<?php echo $obj_base_path->base_path(); ?>/event/<?php echo $eachVal['id'];?>" ><?php if($_SESSION['langSessId']=='eng') { echo stripslashes($eachVal['event_name_en']); } else { echo stripslashes($eachVal['event_name_sp']);}?></a></div>
							<div class="clear"></div>
					...
					?>
						<div class="party_details_box">
							<div class="box1">
								...
								if($objsub_eve->num_rows()){
								?>
                                <p><?php echo date('g:i A',strtotime($first_eve_time)); ?> - <?php echo date('g:i A',strtotime($last_eve_time)); ?></p>
                                <?php
								}
								else{
								?>
							...
							if($objsub_eve->num_rows()){
							?>
							<div class="heading09"><a href="<?php echo $obj_base_path->base_path(); ?>/event/<?php echo $eachVal['id'];?>" ><?php if($_SESSION['langSessId']=='eng') { echo stripslashes($eachVal['event_name_en']); } else { echo stripslashes($eachVal['event_name_sp']);}?></a></div>
                            
                            <?php
								$sub_cnt = 0;
								while($objsub_eve->next_record()){
									list($sub_start_dt,$sub_start_tm) = explode(" ",$objsub_eve->f('event_start_date_time'));
									
									if($sub_cnt<4){
							?>
                            <div class="heading09"><a href="<?php echo $obj_base_path->base_path(); ?>/event/<?php echo $eachVal['id'];?>"><?php if($_SESSION['langSessId']=='eng') { echo substr(stripslashes($objsub_eve->f('event_name_en')),0,30); } else { echo substr(stripslashes($objsub_eve->f('event_name_sp')),0,30);}?></a><span><?php echo date('g:i A',strtotime($sub_start_tm)); ?></span></div>
                            <?php
									}
									else{
									if($sub_cnt==4){
							?>
                            <?php /*?><div class="heading10" onclick="showmoreSubevent('<?php echo $eachVal['id'];?>')"><p>Click to see more</p></div><?php */?>
                            <div class="heading10"><p>more</p></div>
                            <?php
									}
							?>
                            <?php /*?><div style="display:none;" class="more_subevent<?php echo $eachVal['id'];?> heading09"><a href="<?php echo $obj_base_path->base_path(); ?>/event/<?php echo $eachVal['id'];?>"><?php if($_SESSION['langSessId']=='eng') { echo substr(stripslashes($objsub_eve->f('event_name_en')),0,30); } else { echo substr(stripslashes($objsub_eve->f('event_name_sp')),0,30);}?></a><span><?php echo date('g:i A',strtotime($sub_start_tm)); ?></span></div><?php */?>
                            <?php
