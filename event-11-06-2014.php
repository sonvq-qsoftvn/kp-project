...
<meta property="og:title" content="<?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($objEvent->f('event_name_en'))); } else { echo $objEvent->f('event_name_sp');}?>" />

<meta name="title" content="<?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($objEvent->f('event_name_en'))); } else { echo $objEvent->f('event_name_sp');}?>" />

<meta property="og:url" content="<?php echo $actual_link; ?>" />

<meta property="og:description" content="<?php if($_SESSION['langSessId']=='eng') { echo date("l, j F, Y, g:i a",strtotime($objEvent->f('event_start_date_time'))).", ".$obj_venue->f('venue_name').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".htmlentities(stripslashes($objEvent->f('event_short_desc_en'))); } else { setlocale(LC_TIME, 'es_ES'); echo  strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');}?>" />

<meta name="description" content="<?php if($_SESSION['langSessId']=='eng') { echo date("l, j F, Y, g:i a",strtotime($objEvent->f('event_start_date_time'))).", ".$obj_venue->f('venue_name').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".htmlentities(stripslashes($objEvent->f('event_short_desc_en'))); } else { setlocale(LC_TIME, 'es_ES'); echo  strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');}?>" />

<?php if($objEvent->f('event_photo')==''){?>
<meta property="og:image" content="<?php echo $obj_base_path->base_path(); ?>/images/kpassa_logo_fb.png">
<!--<meta property="og:image" content="http://kpasapp.com/files/event/large/13981981061924979_418247818312480_4528237199883652667_n%20(1).jpg"/>-->
