...
/*-------------------------------------------FOR FEES START--------------------------------------------*/
	...
	if($objEvent->f('include_payment') == 1){
		$add_ed_fee_inc_payment_mx = 0.00;
	}
	else  
	{
	  $add_ed_fee_inc_payment_mx = number_format($objfee->f('ticket_min_mx') + (($objfee->f('ticket_percent_nincl')/100)*$_POST['frm_mx_price'.$i]),2,'.',',');
	}
	
	if($objEvent->f('include_promotion') == 1){
		$add_ed_fee_inc_promotion_mx = 0.00;
	}
	else
	{
	 $add_ed_fee_inc_promotion_mx = number_format($objfee->f('promo_fee_min_mx') + (($objfee->f('promo_percent_nincl')/100)*$_POST['frm_mx_price'.$i]),2,'.',',');
	}
	...
	// For Us ===================    
	if($objEvent->f('include_payment') == 1){
		$add_ed_fee_inc_payment_us = 0.00;
	}
	else
	{
       $add_ed_fee_inc_payment_us = number_format($objfee->f('ticket_min_us') + (($objfee->f('ticket_percent_nincl')/100)*$_POST['frm_us_price'.$i]),2,'.',',');
	}
	
	if($objEvent->f('include_promotion') == 1){
		$add_ed_fee_inc_promotion_us = 0.00;
	}
	else
	{
          $add_ed_fee_inc_promotion_us =  number_format($objfee->f('promo_fee_min_us') + (($objfee->f('promo_percent_nincl')/100)*$_POST['frm_us_price'.$i]),2,'.',',');
	}
...
$obj_event_media->all_gallery_for_event($event_id);
...
<meta property="og:title" content="<?php  { echo $objEvent->f('event_name_sp');}?>" />

<meta name="title" content="<?php  { echo $objEvent->f('event_name_sp');}?>" />

<meta property="og:url" content="<?php echo $actual_link; ?>" />

<meta property="og:description" content="<?php  { setlocale(LC_TIME, 'es_ES'); echo  strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');}?>" />

<meta name="description" content="<?php { setlocale(LC_TIME, 'es_ES'); echo  strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');}?>" />
...
-->                <div class="clear"></div>
			...
			<?php $num=$obj_event_media->num_rows();?>
                    <div class="photo_count"><?=FAN_PHOTOS?> (<?php echo $num;?>)</div>
		...
		//echo "<br/>var=".$var;?>
	     ...
	     <!--  <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;" id="media_click"><a href="#media_image<?php echo $k;?>" class="fancybox" id="eventMediaUrl<?php echo $k;?>"><img src="<?php echo $obj_base_path->base_path(); ?>/files/event/gallery/thumb/<?php echo $obj_event_media->f('media_url'); ?>" alt="" width="190" height="143" /></a></li>-->
	     <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;" id="media_click"><a href='<?php echo $obj_base_path->base_path(); ?>/fancy_share.php?event_id=".<?php echo $event_id;?>."&media_id=".<?php echo $media_id;?>."' class="fancybox"><img src="<?php echo $obj_base_path->base_path(); ?>/files/event/gallery/thumb/<?php echo $obj_event_media->f('media_url'); ?>" alt="" width="190" height="143" /></a></li>
	     ...
	     <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;"><a href="<?php echo $obj_event_media->f('media_url');?>" class="fancybox"><iframe width="190" height="143" src="//www.youtube.com/embed/<?php echo end(explode('=',$obj_event_media->f('media_url')));?>" frameborder="0" allowfullscreen></iframe></a></li>
		  <?php }
		  elseif($var=="vimeo") { ?>
		  <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;"><a href="http://vimeo.com/<?php echo  end(explode('/',$obj_event_media->f('media_url')));?>"  class="fancybox"><iframe src="//player.vimeo.com/video/<?php echo  end(explode('/',$obj_event_media->f('media_url')));?>" width="190" height="143" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></a></li>
		  <?php }
		  elseif($var=="dailymotion") {
		    $dm_vid_arr=explode('_',end(explode('/',$obj_event_media->f('media_url'))));
		    $dm_vid = $dm_vid_arr[0];
		    ?>
		 <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;"><a href="http://www.dailymotion.com/embed/video/<?php echo  $dm_vid;?>"  class="fancybox"><iframe frameborder="0" width="190" height="143" src="//www.dailymotion.com/embed/video/<?php echo  $dm_vid;?>" allowfullscreen></iframe></a></li>
		  <?php }?>
	...
	?>
	 <div style="display:none;">
		<div style="width:auto;height:auto; background:#FFF; padding:10px;" id="media_image<?php echo $k;?>">
                <img src="<?php echo $obj_base_path->base_path(); ?>/files/event/gallery/large/<?php echo $obj_event_media->f('media_url');?>"  border="0"  />   
                             <!-----------social share---------->
			     <div class="clear"></div>
	 <div class="fb-share-button" data-href="<?php echo $obj_base_path->base_path()."/event/".$event_id; ?>" data-type="box_count"></div>
		...
		<div style="display:none;">
                                  	<div style="width:auto;height:auto; background:#FFF; padding:10px;" id="media_url<?php echo $k;?>">
                                    	      <object width="560" height="315"><param name="movie" value="//www.youtube.com/v/opj24KnzrWo?version=3&amp;hl=en_US"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="//www.youtube.com/v/<?php echo end(explode('=',$obj_event_media->f('media_url')));?>?version=3&amp;hl=en_US" type="application/x-shockwave-flash" width="560" height="315" allowscriptaccess="always" allowfullscreen="true"></embed></object>
                                    </div>
                                  </div>
	 <!---------------Fancy  box  start---------------------->
...
<!----------------Fancy box end--------------------->
		...
		//$k++;
			...
			?>
			 ...
			 <div style="display:none;">
		<div style="width:auto;height:auto; background:#FFF; padding:10px;" id="media_image">
                <img src="<?php echo $obj_base_path->base_path(); ?>/files/event/gallery/large/<?php echo $obj_event_media->f('media_url');?>"  border="0"  />   
                                    </div>
	    </div>
