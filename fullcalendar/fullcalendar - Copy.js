...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		if (showWeekNumbers) {
			s += "<th class='fc-agenda-axis fc-week-number " + headaerClass + "'/>";
		}
		else {
		...
		for (i=0; i<colCnt; i++) {
			s +=
				...
				"<div style='height: auto; min-height: 565px; overflow: hidden; margin: display: inline-block; 8px auto;'>" +
		...
		for (i=0; d < maxd; i++) {
			...
			s +=
				"<tr class='fc-slot" + i + ' ' + (!minutes ? '' : 'fc-minor') + "' style='display:none;'>" +
