...
$objevent_category_sub=new user;
...
//check for perticular subevents
$objsub_event_click->sub_event_click($event_id,$sub_id);
$objsub_event_click->next_record();
//echo "<pre>";print_r($objsub_event_click); exit;
$l=2;
...
list($sub_event_date_new,$sub_event_time_new) = explode(" ",$obj_chk->f('event_start_date_time'));
list($sub_event_end_new,$sub_event_time_end_new) = explode(" ",$obj_chk->f('event_end_date_time'));

//echo "hii".$sub_event_date_new; exit;
...
if(isset($_POST['action']) && $_POST['action'] == 'cart')	
{
	...
	echo $_POST['frm_us_price'.$i]."<br />";*/
		if($_POST['frm_ticket'.$i] != ''){
			$cid[] = $obj_cart->add_to_cart_sub($_POST['frm_event_id'],$_POST['frm_sub_id'],$_POST['frm_ticket'.$i],$_POST['frm_mx_price'.$i],$_POST['frm_us_price'.$i],$unique,$_POST['frm_us_tid'.$i],$_POST['frm_payment'],$_POST['frm_date'],$_POST['frm_end_date'],$_POST['frm_start'],$_POST['frm_end'],$_SESSION['session_id']);
		}
	}
	...
	{*/
		//$_SESSION['cid'] = '';
		//header("location: ".$obj_base_path->base_path()."/payment/".$_POST['frm_event_id']);  ***imp
	//}
	/*print_r($cid);
	exit;*/
}
...
//================================  for meta====================
//exit;
//echo  "pic= ". $objsub_event_click->f('event_photo'); exit;
?>
...
<?php  }
 ...
 {
	?>
<title><?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($obj_chk->f('event_name_en'))); } else { echo htmlentities(stripslashes($obj_chk->f('event_name_sp')));}?></title>
<meta charset="utf-8">
<meta name="title" content="<?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($obj_chk->f('event_name_en'))); } else { echo htmlentities(stripslashes($obj_chk->f('event_name_sp')));}?>">
<meta name="keywords" content="<?php  if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($objevent_category->f('category_name'))); } else { echo htmlentities(stripslashes($objevent_category->f('category_name_sp')));}?>">
<meta name="description" content="<?php  if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($obj_chk->f('event_short_desc_en'))); } else { echo htmlentities(stripslashes($obj_chk->f('event_short_desc_sp')));}?>">
...
<meta property="og:title" content="<?php if($_SESSION['langSessId']=='eng') { echo stripslashes($obj_chk->f('event_name_en')); } else { echo stripslashes($obj_chk->f('event_name_sp'));}?>" />
<meta property='og:site_name' content='Kpasapp' />

<meta name="title" content="<?php if($_SESSION['langSessId']=='eng') { echo stripslashes($obj_chk->f('event_name_en')); } else { echo stripslashes($obj_chk->f('event_name_sp'));}?>" />

<meta property="og:url" content="<?php echo $actual_link; ?>" />
<meta property="og:description" content="<?php if($_SESSION['langSessId']=='eng') { echo $obj_chk->f('event_short_desc_en'); } else { echo $obj_chk->f('event_short_desc_sp');}?>" />
...
function addtocart(num,mx,us,tid,event_id,dd,ed,st,et)
{
	//alert(dd);
	//alert(num)
	$("#frm_event_id").val(event_id);
...
function cart() {
	...
	$.ajax({ 
		   url: "<?php echo $obj_base_path->base_path(); ?>/ajax_folder/subevent_ajax.php",
		   cache: false,
		   type: "POST",
		   data: $('#frm').serialize(),   
		   success: function(data){
			   //alert(data);
			   $("#items").html(data);
			   //$(".cart_new option:selected").val();
			   //window.location.reload(true);
			   $('.cart_new').find('option:first').attr('selected', 'selected');
			   resetForm($('#frm'));
		   }
	});
}
...
function save(type){
	...
	//window.location.href = '<?php echo $obj_base_path->base_path()?>/sub_payment/<?php echo $event_id;?>';
	
	//window.open('<?php echo $obj_base_path->base_path()?>/sub_payment/<?php echo $event_id;?>'); 
	
	window.parent.location.href = '<?php echo $obj_base_path->base_path()?>/sub_payment/<?php echo $event_id;?>/sub_id/<?php echo $sub_id;?>';
...
<!--                	<span style="float: right; margin: 0 auto; padding: 0; width: 160px;">
			...
			<?php
					...
					{
					?>
			<div class="heading_top"><h1>Seleccione su funciÃ³n</h1></div>
					<?php
						} 
					}
					?>
...
-->                        </div>
			...
			<div class="btn2_sudip"><a onclick="scroll_to();"><?php if($_SESSION['langSessId']=='eng'){?> More <?php }else{?> Mas <?php }?></a></div>
			    ...
			    <div style="margin:10px 0 20px; font-weight:bold;">
						...
						<?php
						   // print_r("p_venue=".$obj_venue->f('venue_name'));
						   // print_r("ch_venue=".$obj_venue_sub->f('venue_name'));
						//if($obj_venue->f('venue_name')!=$obj_venue_sub->f('venue_name'))
						//{
							?>
						<?php $add = $obj_venue_sub->f('venue_name').", ".$obj_venue_sub->f('venue_address').", ".$obj_venue_sub->f('city').", ".$obj_venue_sub->f('st_name'); ?> 
...
function createInfoWindowContent() {
  ...
  <?php 
  $addressWithBreak = str_replace(",",", <br />",urldecode($Address));
  $addressTrimed = trim(preg_replace('/\s+/', ' ', $addressWithBreak));
  ?>
  return "<?php echo $addressTrimed; ?>";
}
...
-->                          <!--<div id="map" style="width:323px; height:325px; font-family: arial; font-size: 12px; color: #313E61; text-align: center; background-color:#FFFFFF;"></div>-->    
			...
			<form name="frm" id="frm" method="post">
                        ...
                        <input type="hidden" name="frm_sub_id" id="frm_sub_id" value="<?php echo $sub_id;?>" />
			...
			<div class="select_box1">
					...
					$count=1;
                              ...
                              <tr>
                                <td width="10%">
                                    <div class="dropdown25"><select class="cart_new" name="ticket_num" id="ticket_num<?php echo $count;?>" onchange="addtocart(<?php echo $count;?>,<?php echo $obj_sub_ticket->f('price_mx');?>,<?php echo $obj_sub_ticket->f('price_us');?>,<?php echo $obj_sub_ticket->f('ticket_id');?>,<?php echo $event_id;?>,'<?php echo date("d-m-Y",strtotime($sub_event_date_new));?>','<?php echo date("d-m-Y",strtotime($sub_event_end_new));?>','<?php echo date('g:i A',strtotime($sub_event_time_new));?>','<?php echo date('g:i A',strtotime($sub_event_time_end_new));?>');">
                                    <?php for($i=0;$i<=$obj_sub_ticket->f('ticket_num');$i++) {?>
                                        <option value="<?php echo $i;?>"><?php echo $i;?></option>
                                    <?php } ?>
                                    </select></div>
				    <input type="hidden" class="myClass" name="frm_ticket<?php echo $count;?>" class="tick" id="frm_ticket<?php echo $count;?>" value="" />
                                    <input type="hidden" class="myClass" name="frm_mx_price<?php echo $count;?>" id="frm_mx_price<?php echo $count;?>" value="" />
                                    <input type="hidden" class="myClass" name="frm_us_price<?php echo $count;?>" id="frm_us_price<?php echo $count;?>" value="" />
                                    <input type="hidden" class="myClass" name="frm_us_tid<?php echo $count;?>" id="frm_us_tid<?php echo $count;?>" value="" />
                                </td>
				    ...
				    <?php
                        ...
                        <?php  if($obj_sub_ticket->num_rows()){?>
			<div class="select_box2">
                             <?php if($_SESSION['langSessId']=='eng') {?>
				<a href="javascript:void(0);" onclick="cart();">Add To Cart</a>
				<div id="items">
					<table border="0">
						<?php
						$obj_ticket->getTicket_sub($_SESSION['ses_admin_id'],$_SESSION['unique'],$sub_id);
						...
						?>
                                ...
                                <div><a href="javascript:void(0);" id="save" onclick="save();"><img src="<?php echo $obj_base_path->base_path(); ?>/images/reserv_btn.gif" /></a></div>
                             <?php } else {?>   
                                <div><a href="javascript:void(0);" id="save" onclick="save();"><img src="<?php echo $obj_base_path->base_path(); ?>/images/spainreser_btn.gif" /></a></div>
                             <?php } ?>  
                         </div>
			 <?php } ?>			
						<?php }
                        ...
                        <div class="offer_box" style="float: left; margin: 0; width:100%;">                       
                         ...
                         <div class="imgbox" style="width:100%; height: auto;">
                           ...
                           <!-- <li><img src="<?php //echo $obj_base_path->base_path(); ?>/images/preview_img1.gif" border="0" /></li>-->
                            ...
                            <?php if($objsub_event_click->f('event_photo')){  

							?>
                            	<li style="margin: 0; display: block;">
                                <a href="#feature_image" id="feature"><img src="<?php echo $obj_base_path->base_path(); ?>/files/event/medium/<?php echo $objsub_event_click->f('event_photo');?>"  border="0"  /></a>
                                </li>
                                 <script type="text/javascript">
									$(document).ready(function() {
										$("#feature").fancybox({ 
                                  ...
                                  <div style="display:none;">
                                  	<div style="width:auto;height:auto; background:#FFF; padding:10px;" id="feature_image">
                                    	<img src="<?php echo $obj_base_path->base_path(); ?>/files/event/large/<?php echo $objsub_event_click->f('event_photo');?>"  border="0"  />
                                    </div>
                                  </div>
				<?php
                                  }
