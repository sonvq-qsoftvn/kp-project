			...
			echo "</pre>";*/
								...
								foreach($allData as $eachData){
									...
									if($row[$i]['multi_event_date'] !=$row[$i-1]['multi_event_date']  && $row[$i]['multi_event_date']!=0){
										...
										$obj_samefunc->multi_event_datewise($event_id,$row[$i]['multi_event_date'],$date2);
							?>
                                ...
                                <?php 
									...
									if($objmul_date->num_rows()){
										...
										$cityData = '';
										$venueData = '';
										while($obj_samefunc->next_record()){
										$kk++;
										//echo "xxx ".$objmul_date->f('multi_id')."amit";exit;
											if($obj_samefunc->f('multi_id')==$eachData['multi_id']){
												//continue;
											}
											else{
												//echo "blah";
												list($new_date,$new_time) = explode(" ",$obj_samefunc->f('multi_start_time'));
												list($new_end_date,$new_end_time) = explode(" ",$obj_samefunc->f('multi_end_time'));
												//echo "<br>".$i."<br>".$row[$i]['multi_start_time']."<br>".$new_time."<br>";
												//if($row[$i]['city']!=$objmul_date->f('city_name') && !in_array($objmul_date->f('city_name'),$cityData))
								...
								?>
                                            ...
                                            <p style="cursor:pointer;" class="abc" id="tbl1<?php echo $kk;?>" onclick="setHover1(<?php echo $kk;?>,'<?php echo date("D",strtotime($multi_event_date))." ".date("M",strtotime($multi_event_date))." ".date("d",strtotime($multi_event_date)).", ".date("Y",strtotime($multi_event_date))." - ".date('g:i A',strtotime($new_time))." to ".date('g:i A',strtotime($new_end_time));?>','<?php echo $obj_samefunc->f('multi_id');?>','<?php echo $event_id;?>')">
												<?php if($multi == 0){echo date('g:i A',strtotime($multi_event_time));}else{echo date('g:i A',strtotime($new_time));} ?>
                                            </p>
                                <?php				

												$cityData[] = $obj_samefunc->f('city_name');
												$venueData[] = $obj_samefunc->f('venue_name');
