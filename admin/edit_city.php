...
if(isset($_POST['addcat']) && $_POST['addcat'] == '1')	
{
	...
	$obj_edit->updateCity($county_id,$city_name,$city_name_sp,$city_id);
	$_SESSION['msg'] = "City added successfully.";
	header("Location:".$obj_base_path->base_path()."/admin/edit_city/".$city_id);
	exit;
}

$obj_gt_dtls->geteachCityDtls($city_id);
$obj_gt_dtls->next_record();
                    ...
                    <tr>
                     ...
                     <tr>
                        <th colspan="2">Country<br/>
                            ...
                            ?>
                            <option value="<?php echo $obj_country->f('id'); ?>" <?php if($obj_gt_dtls->f('country_id')==$obj_country->f('id')){?> selected="selected"<?php } ?>><?php echo stripslashes($obj_country->f('nicename')); ?></option>
                            <?php } ?>
                              </select></span>
                        </th>
                      </tr>
                     <tr>
                        <th colspan="2">State<br/>
                            ...
                            <option value="">Select</option>
                            <?php 
                            $obj_state->getStateByCountry($obj_gt_dtls->f('country_id'));
                            while($obj_state->next_record()){
                            ?>
                            <option value="<?php echo $obj_state->f('id'); ?>" <?php if($obj_gt_dtls->f('stateid')==$obj_state->f('id')){?> selected="selected"<?php } ?>><?php echo stripslashes($obj_state->f('state_name')); ?></option>
                            <?php } ?>
                            </select></span>
                     ...
                     <tr>
                        <th colspan="2">County<br/>
                            ...
                            <option value="">Select</option>
                            <?php 
                            $obj_county->obj_county($obj_gt_dtls->f('stateid'));
                            while($obj_county->next_record()){
                            ?>
                            <option value="<?php echo $obj_county->f('id'); ?>" <?php if($obj_gt_dtls->f('countyid')==$obj_county->f('id')){?> selected="selected"<?php } ?>><?php echo stripslashes($obj_county->f('county_name')); ?></option>
                            <?php } ?>
                             </select></span>
                      ...
                      <tr>
						<th width="16%">
                        	...
                        	<span class="event_fieldbg20"><input type="text" name="city_name" id="city_name" value="<?php echo $obj_gt_dtls->f('city_name');?>" class="event_field" style="height: 25px; margin: 5px 0; width: 290px;"/></span>
                       </th>
                      </tr>
                      <tr>                                                      
                        <th>
                        <span class="state_sp">SP</span><br/>
                        <span class="event_fieldbg20"><input type="text" name="city_name_sp" id="city_name_sp" value="<?php echo $obj_gt_dtls->f('city_name_sp');?>" class="event_field" style="height: 25px; margin: 5px 0; width: 290px;"/></span>
                      	</th>
                      </tr>
                      
                      <tr>                                                        
                        <th colspan="2"><input type="hidden" name="addcat" value="1" /><input type="submit" name="submit" id="submit" value="Save" class="createbtn"/></th>
                      </tr>
                    </table>
                  </div>
                </div>											
