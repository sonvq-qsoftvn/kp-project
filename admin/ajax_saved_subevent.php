...
//echo "Hi ".$event_id; exit;
//echo $_SESSION['unique_id'];


	// Check if the event is already save or not
	$obj_check_saved->checkSavedSubEvent($_SESSION['unique_id_subevent']);
	if($obj_check_saved->num_rows())
	{
		...
		//$obj_edit_saved->editSavedSubEvent($_SESSION['ses_user_id'],$event_name_sp,$event_name_en,$short_desc_sp,$short_desc_en,$event_start_date_time,$event_start_ampm,$event_end_date_time,$event_end_ampm,$venue_state,$venue_county,$venue_city,$venue,$page_content_en,$page_content_sp,$event_tag,$file_name,$identical_function,$recurring,$sub_events,$Paypal,$Bank,$Oxxo,$Mobile,$Offline,$publish_date,$event_time,$event_time_period,$r_month,$r_month_day,$mon,$tue,$wed,$thu,$fri,$sat,$sun,$r_span_start,$r_span_end,$event_start,$event_end,$all_day,$event_lasts,$attendees,$invitation_only,$password_protect_check,$pass_protected,$radio_access,$pay_ticket_fee,$promo_charge,$paper_less_mob_ticket,$print,$will_call,$status,$_SESSION['unique_id_subevent'],$privacy,$event_id);
		

		// Delete previous category..
		$obj_del_saved_cat->del_sub_event_cat($obj_check_saved->f('event_id'));
		...
		if(count($finalArray)>0)
		   ...
		   {
				$checkCnt = $obj_duplicate->chkExistsubcategory($obj_check_saved->f('event_id'),$finalArray[$a]);
				if(!$checkCnt){
					$obj_saved_category->addSavedSubCat($finalArray[$a],$obj_check_saved->f('event_id'),$event_id);
				}
			}
		}
		
		// Delete previous Event type..
		$obj_del_eventtype->del_sub_event_type($obj_check_saved->f('event_id'));
		
		// Add Event type
		if(count($event_types)>0)
   		{
		   $obj_eventtype->Add_auto_sub_save_eventtype($event_types,$obj_check_saved->f('event_id'),$event_id);
		}		
	...
	else 
	{
		...
		// Add Event
        $objectClientPdo = new pdoDatabase();
        $last_event_id = $objectClientPdo->addSubEvent($event_name_sp,$event_name_en,$short_desc_sp,$short_desc_en,$event_start_date_time,$event_start_ampm,$event_end_date_time,$event_end_ampm,$venue_state,$venue_county,$venue_city,$venue,$page_content_en,$page_content_sp,$event_tag,$file_name,$identical_function,$recurring,$sub_events,$Paypal,$Bank,$Oxxo,$Mobile,$Offline,$publish_date,$event_time,$event_time_period,$r_month,$r_month_day,$mon,$tue,$wed,$thu,$fri,$sat,$sun,$r_span_start,$r_span_end,$event_start,$event_end,$all_day,$event_lasts,$attendees,$invitation_only,$password_protect_check,$pass_protected,$radio_access,$pay_ticket_fee,$promo_charge,$paper_less_mob_ticket,$print,$will_call,$event_id,$status,$_SESSION['unique_id_subevent'],$privacy,$event_id);
		//$last_event_id = $obj_add->addSubEvent($event_name_sp,$event_name_en,$short_desc_sp,$short_desc_en,$event_start_date_time,$event_start_ampm,$event_end_date_time,$event_end_ampm,$venue_state,$venue_county,$venue_city,$venue,$page_content_en,$page_content_sp,$event_tag,$file_name,$identical_function,$recurring,$sub_events,$Paypal,$Bank,$Oxxo,$Mobile,$Offline,$publish_date,$event_time,$event_time_period,$r_month,$r_month_day,$mon,$tue,$wed,$thu,$fri,$sat,$sun,$r_span_start,$r_span_end,$event_start,$event_end,$all_day,$event_lasts,$attendees,$invitation_only,$password_protect_check,$pass_protected,$radio_access,$pay_ticket_fee,$promo_charge,$paper_less_mob_ticket,$print,$will_call,$event_id,$status,$_SESSION['unique_id_subevent'],$privacy,$event_id);
		
		//echo $_SESSION['subevent_id'];
		...
		if(count($finalArray)>0)
		   ...
		   for($a=0;$a<count($finalArray);$a++)
		   {
				$checkCnt = $obj_duplicate->chkExistsubcategory($last_event_id,$finalArray[$a]);
				if(!$checkCnt){
					$obj_saved_category->addSavedSubCat($finalArray[$a],$last_event_id);
				}
			}
		}		
		...
		if(count($event_types)>0)
   		{
		   $obj_eventtype->Add_auto_sub_save_eventtype($event_types,$last_event_id,$event_id);
		}		
