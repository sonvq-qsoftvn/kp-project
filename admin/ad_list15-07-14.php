...
$(document).ready(function(){
	...
	if ($("#add_short_by" ).val()!="") {	
		...
		?>
	       var  url = "<?php echo $obj_base_path->base_path();?>/admin/ad_list.php?short_by="+short_by_val+"&add_then_by=<?php echo $_REQUEST['add_then_by']?>"+"&page=<?php echo $_REQUEST['page']?>";
	       <?php
			}
			else{
		?>
		var  url = "<?php echo $obj_base_path->base_path();?>/admin/ad_list.php?short_by="+short_by_val+"&add_then_by=<?php echo $_REQUEST['add_then_by']?>";	
...
<div id="maindiv">
   ...
   <div class="body_bg">
     ...
     <div style="width:20px; padding: 5px; float:left"> <input type="button" onclick="clear_url()" value="Clear"/> </div>
