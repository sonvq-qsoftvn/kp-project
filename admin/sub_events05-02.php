...
<?php include("admin_header.php"); ?>
  <div id="maindiv">
	...
	<tr><td colspan="7" align="center" style="padding-top:10px;"><strong><font color="#0094A4">Set program for</font></strong></td></tr>
	<?php 
	$obj->fetchEventById($obj_parent->f('event_id'));
    $obj->next_record();
	?>
	<tr>
		<td width="20%" class="top_txt">Event</td>
		<td width="20%" class="top_txt">City</td>
		<td width="20%" class="top_txt">Venue</td>
		<td width="20%" class="top_txt">Start Date</td>
	...
	<tr>
		...
		<td width="15%" class="top_txt">City</td>
		<td width="15%" class="top_txt">Venue</td>
		<td width="16%" class="top_txt">Start Date</td>
		<td width="16%" class="top_txt">End Date</td>
		<td width="10%" class="top_txt">Edit</td>
		<td width="10%" class="top_txt">Delete</td>
	</tr>
	<?php
...
?>
	<tr>
		...
		<td width="18%"><a href="javascript:void(0);" onclick="deleteSubEvent('<?php echo $objlist->f('event_id')?>','<?php echo $obj_parent->f('event_id');?>');"><img src="<?php echo $obj_base_path->base_path(); ?>/images/cross.gif" alt="" width="20" height="16" /></a></td>
	</tr>
	    <?php
		}
	}
	...
	<div style="text-align:center;margin-bottom: 20px;margin-top: 20px;">
	<h2>Create/Edit Show - Sub Event</h2>
	</div>
	...
	<br/>
		<span class="event_fieldbg8"><textarea name="short_desc_sp" class="event_field" style="width: 290px; margin: 5px 0; padding: 3px 5px; height: 60px;" onblur="saveAutoEvent();"><?php echo $short_desc_sp;?></textarea></span>
	</div>
	...
	<br/>
	<span class="event_fieldbg8">
	<textarea name="short_desc_en" class="event_field" style="width: 290px; margin: 5px 0; padding: 3px 5px; height: 60px;" onblur="saveAutoEvent();"><?php echo $short_desc_en;?></textarea>
	</span>
    </div>
    </div>
    ...
    <tr>
      ...
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td style="color:#FF0000; padding: 0;"><strong>/</strong></td>
            <td style="color:#FF0000; padding: 0 0 0 13px;"><strong>YYYY<?php //echo $st_eve_hr;?></strong></td>
        </tr>
        <tr>
          <td><input type="text" name="event_day_st" id="event_date_st" value="<?php if($st_eve_dy!=""){echo $st_eve_dy;}else{echo 00;}?>"  class="textbg_grey"  style="width: 30px;" /></td>
          <td>/</td>
          <td><input type="text" name="event_month_st" id="event_month_st" value="<?php if($st_eve_mn!=""){echo $st_eve_mn;}else{echo 00;}?>" class="textbg_grey"  style="width: 30px;" /></td>
          <td>/</td>
          <td><input type="text" name="event_year_st" id="event_year_st"  value="<?php if($st_eve_yr!=""){echo $st_eve_yr;}else{echo 0000;}?>" class="textbg_grey"  style="width: 40px;" /></td>
          <td></td>
        </tr>
        <tr>
      ...
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin: 20px 0 0 0;">
        <tr>
          ...
          <td>/</td>
          <td><input type="text" name="event_month_end" id="event_month_end" value="<?php if($end_eve_mn!=""){echo $end_eve_mn;}else{echo 00;}?>"  class="textbg_grey" style="width: 30px;"/></td>
          <td>/</td>
   ...
   <?php /*?> <textarea name="page_content_sp" cols="35" rows="10"><?php if($msg!="" && $page_content_sp!=""){echo $page_content_sp;}?></textarea><?php */?>
    ...
    <div class="event_ticket" style="background:none;">
		...
		<span style="float: left;">
        	<p class="rad_button"><input type="radio" name="privacy" id="public_privacy" value="0"  onclick="privacy_policy(); saveAutoEvent();" checked="checked" /> Public</p>	
			<p class="rad_button"><input type="radio" name="privacy" id="private_privacy" value="1" onclick="privacy_policy(); saveAutoEvent();" <?php if($set_privacy == 1){echo "checked";}?> /> Private</p>
         </span>
		<span id="public_content" <?php if($set_privacy == 0){?>style="display:block;"<?php }else{ ?>style="display:none;"<?php }?>>This event is public and will be listed in our event directory and submited to search engines</span>
		<span id="private_content" <?php if($set_privacy == 1){?>style="display:block;"<?php }?> >
          <table width="100%" border="1">
              ...
              <tr>
                <td><input type="checkbox" name="attendees" id="attendees" value="1" onclick="saveAutoEvent();" <?php if($attendees_share == 1){echo "checked";}?>  />&nbsp; Attendees can share the event with their friends on socail networks.</td>
              </tr>
              <tr>
                <td><input type="checkbox" name="invitation_only" id="invitation_only" value="1" onclick="saveAutoEvent();" <?php if($attendees_invitation == 1){echo "checked";}?> />&nbsp; Invitation Only - Attendees must receive a KPasapp invitation to register.</td>
              </tr>
              <tr>
                <td><input type="checkbox" name="password_protect_check" id="password_protect_check" value="1" onclick="saveAutoEvent();" <?php if($password_protect == 1){echo "checked";}?> />&nbsp; Password Protected &nbsp; 
    ...
    <div>
     <ul>
        <li><input name="radio_access" type="radio" value="1" <?php if($radio_access==1){?> checked="checked"<?php } ?> onclick="showticketingsection(); saveAutoEvent();"  /><strong>Some or all access to this event requires reservation, ticketing and/or RSVP</strong></li>
		<li><input name="radio_access" type="radio" value="2" <?php if($radio_access==2){?> checked="checked"<?php } ?> onclick="showticketingsection(); saveAutoEvent();"/><strong>Access to this sub-event is included in at least one of the master events tickets</strong></li>
        <li><input name="radio_access" type="radio" value="0" <?php if($radio_access==0){ echo 'checked="checked"'; } ?> onclick="showticketingsection(); saveAutoEvent();"  /><strong>All access to this event is open to the public free of charge and without reservation</strong></li>
    ...
    <div id="ticket_area" style="display:none;">
      ...
      <input type="hidden" name="exit_ticket_id" id="exit_ticket_id" value="0" />
       ...
       <div class="event_date8">
        ...
        <div class="event_date9">
        	Number of available seats <input type="text" name="ticket_num" id="ticket_num" style="width: 64px;" value="<?php echo $ticket_num;?>"/> 
        	From <input type="text" name="from_ticket" id="from_ticket" value="<?php echo $from_ticket;?>" /> 
            To <input type="text" name="to_ticket" id="to_ticket" value="<?php echo $to_ticket;?>" />
        </div>	
        <div class="event_date9">
        	Early bird discount <input type="text" name="eairly_dis_percen" id="eairly_dis_percen" value="<?php echo $eairly_dis_percen;?>" /> 
        	% Up to <input type="text" name="eairly_days" id="eairly_days" style="width: 64px;" value="<?php echo $eairly_days;?>"/> days before the event
        </div>
        <div class="event_date9">
        	Group discount <input type="text" name="group_dis_per" id="group_dis_per" value="<?php echo $group_dis_per;?>" /> 
            % over <input type="text" name="group_dis_days" id="group_dis_days" style="width: 64px;"  value="<?php echo $group_dis_days;?>"/> tickets
        </div>
        <div class="event_date9">Membership only
        	<span class="membershipClass">
                ...
                <input name="members_only" id="members_only2" type="radio" value="1" checked="checked"  style="margin: 5px;" /> No 
            </span>
        </div>
	...
	<div class="event_ticket" style="background: none; width: 702px; border: 0; margin: 0 auto 0 10px;">
	 ...
	 <div class="event_ticketr" style="margin: 0 auto 0 11px; width: auto; padding-right: 15px;">
        <h1>
        <input type="submit" name="publish" value="Save And Create New Sub-Event" onclick="return complete();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
        <input type="button" name="publish" value="Save And Duplicate Sub-Event" onclick="return complete1();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
        <input type="button" name="publish" value="Save And Exit" onclick="return save_exit();" class="event_save" style="padding:0px; height: auto; cursor:pointer;" /><br />
        <input type="button" name="publish" value="Discard And Exit" onclick="return discard_save();" class="event_save" style="padding:0px; height:auto;cursor:pointer;" /><br />
        </h1>
	  	<div class="clear"></div>
	</div>
	<div class="clear"></div>
    ...
    <div class="myevent_right">
        ...
        <h1>
            <input type="button" name="publish" value="Save And Create New Sub-Event" onclick="return complete();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
            <input type="button" name="publish" value="Save And Duplicate Sub-Event" onclick="return complete1();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
            <input type="button" name="publish" value="Save And Exit" onclick="return save_exit();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
            <input type="button" name="publish" value="Discard And Exit" onclick="return discard_save();" class="event_save" style="padding:0px; height: auto;cursor:pointer;" /><br />
        </h1>
...
function checkCat(category_id)
{
                ...
                <ul>
                 <?php
                    ...
                    <li style="padding-left: 8px;">
                    	<input type="checkbox" name="maincat[]" id="maincat<?php echo $objlist_cat->f('category_id');?>" value="<?php echo $objlist_cat->f('category_id');?>" class="category_1" onclick="showSubCat(<?php echo $objlist_cat->f('category_id');?>); saveAutoEvent();" <?php if(in_array($objlist_cat->f('category_id'),$arrMaincat)) { echo 'checked'; }?> /><?php echo $objlist_cat->f('category_name'); if($obj_subcat->num_rows()){?> 			  
                    ...
                    <?php
					 ...
					 ?>
                            ...
                            <li style="padding-left:8px;"><input onclick="checkCat(<?php echo $objlist_cat->f('category_id');?>)" type="checkbox" name="maincat[]" value="<?php echo $obj_subcat->f('category_id');?>" onblur="saveAutoEvent();" <?php if(in_array($obj_subcat->f('category_id'),$arrMaincat)) { echo 'checked'; }?> /><?php echo $obj_subcat->f('category_name');?></li>
                         <?php
								}
							}
						?></ul>	
	...
	<div class="eventag_box">
		...
		<div class="clear"></div>
		<div><input type="text" name="event_tag" id="event_tag" value="<?php echo $event_tag;?>" class="textbg_add"/> <input type="button" name="save" value="Add" class="btn_add" onblur="saveAutoEvent();" /></div>
		<div class="clear"></div>
