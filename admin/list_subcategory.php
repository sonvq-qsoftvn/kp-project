...
$sc_delete = new merchant_admin;
$obj_parent = new merchant_admin;
//$pag1=new pagination1;
	
//===============CODE FOR DELETE===================//
if(isset($_GET['action']) && $_GET['action'] == "delete")	
{ 
	$sc_delete->delsubcat($_GET['subcatid']);
...
function del(scid)
{
	...
	{
		location.href="list_subcategory.php?action=delete&subcatid="+scid;
	}
...
<?php include("admin_header.php"); ?>
  <div id="maindiv">
    ...
    <!--start body-->
      <div id="body">
        <div class="body2"> 
          <div class="clear"></div>
           <div class="blue_box1">
           <div class="blue_box10"><p style="font-size:28px;">List Sub-Category</p></div>
    ...
    <table cellpadding="0" cellspacing="0" width="100%"   class="id_detail2">
                                    ...
                                    ?>
                                    <?php
                                        if($_REQUEST['err']==2 || $_SESSION['cat_edit']!="")
                                        {
                                    ?>
                                    <td colspan="7" style="font: bold 13px/30px Arial, Helvetica, sans-serif; padding: 2px 0; text-align: center; color:#000; background: #ccc;">
                                    	<?php if($_SESSION['cat_edit']) { echo $_SESSION['cat_edit']; $_SESSION['cat_edit'] = ''; } else { ?>
                                ...
                                <tr>
                                    ...
                                    <th class="top_txt">Parent Category</th>
									...
									$objlist->getAllSubCat($limit);
									$objlist_num->getAllSubCatnum();
									...
									if($num>0)
									{
										...
										while($row = $objlist->next_record())
										{
											...
											// Get Parent Category
											$obj_parent->getParentCatName($objlist->f('category_id'));
											$obj_parent->next_record();
								?>
								<tr>         
								  <td width="8%" style="padding: 0 10px;"><?php echo $cnt;?></td>
								  <td width="24%"><?php echo $obj_parent->f('cat1');?></td>
								  <td width="23%"><?php echo $objlist->f('category_name');?></td>
								  <td width="11%">
                                  <img src="<?php echo $obj_base_path->base_path(); ?>/images/edit.gif" alt="" width="20" height="16" />
                                  <a href="<?php echo $obj_base_path->base_path(); ?>/admin/edit_sub_category/<?php echo $objlist->f('category_id');?>">Edit</a>
								 ...
								 <?php $cnt++; }?>
								  <tr>
								  <td colspan="6">
								  <div style="width: 150px; float:right; margin: -20px 0 10px 0; font: normal 11px/18px Arial, Helvetica, sans-serif;"> <?php $p->show();?> </div>
								  </td>
								  </tr>
								<?php
									}
