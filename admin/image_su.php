...
$(function(){
		...
		new AjaxUpload(btnUpload, {
			action: '<?php echo $obj_base_path->base_path(); ?>/admin/uploadPhotoEdit1.php',
			name: 'uploadfile',
			...
			onComplete: function(file, response){
				...
				$('#imgid').html('<img src="<?php echo $obj_base_path->base_path(); ?>/files/test/thumb/'+response+'" />')
				$('#me').html('');
...
function media_library()
{
    ...
    <div class="event_ticket" style="background:none;">
		...
		<span style="float: left;">
			...
			{
			?>
		<p class="rad_button"><img style="margin-left: 2px; margin-top: 7px;" src="<?php echo $obj_base_path->base_path(); ?>/images/radio.png" alt="" border="0" onclick="func_type();"/> Public</p>	
			<?php
			}
			?>
			<p class="rad_button"><input type="radio" name="privacy" id="private_privacy" value="1" onclick="privacy_policy(); saveAutoEvent();" <?php if($obj_admin->f('account_type') == 0 || ($obj_admin->f('account_type') == 1 && $obj_admin->f('prof_complete') == 0)){ echo "checked";} ?> /> Private</p>
			</span>
		<?php
		}
		?>
	...
	<h1><a href="javascript:media_library()" onclick="">Set Feature Image And Event Gallery<img src="<?php echo $obj_base_path->base_path(); ?>/images/question_mark.gif" alt="" width="31" height="28" border="0"/></a></h1>
	    <span>The feature image is used by social networks. It is highly recommended to set it for your event.
You can also add photos or videos to your media library.</span>
