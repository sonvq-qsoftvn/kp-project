...
//$page_content_sp = addslashes($_POST['page_content_sp']);
 $page_content_en = addslashes($_POST['event_des_en']);
 $page_content_sp = addslashes($_POST['event_des_sp']);

//echo "p_c_e= ".htmlentities($page_content_en);
//echo "pc_s= ".htmlentities($page_content_sp);
...
$eve_id = $_POST['id'];
	...
	$fetch_temp_tickets_num=$obj_temp_tickets_call->fetch_temp_tickets($_POST['id']);
		if(($fetch_temp_tickets_num!=0 && $_POST['ticket_buy']==1) || $_POST['ticket_buy']==0)
		{
			// Delete previous Tickets..
			$obj_del_saved_cat->deleteSavedTickets($_POST['id']);
			// Update Event Tickets
			$obj_add_ticket->addSavedTickets($_POST['id']);
			
			// Update event Id
			$obj_edit_ticket->editSavedTicketByEvent($_POST['id'],$obj_check_saved->f('event_id'));
		}
