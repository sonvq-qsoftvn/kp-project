...
<?php
// Get Parent Event
$obj_parent->fetch_event_id($_GET['event_id']);
...
$obj_eve_tic->allTicketListCount($_GET['event_id']);
...
//Sub Event Date Time
$event_start_date_time_sub = $obj_subparent->f('event_start_date_time');
...
<?php include("admin_header.php"); ?>
  <div id="maindiv">
    ...
    $obj->next_record();
	list($main_event_date,$lain_event_time) = explode(" ",$obj->f('event_start_date_time'));
	list($main_eve_yr,$main_eve_mon,$main_eve_day) = explode("-",$main_event_date);
	
	list($main_event_date_end,$lain_event_time_end) = explode(" ",$obj->f('event_end_date_time'));
	list($main_eve_yr_end,$main_eve_mon_end,$main_eve_day_end) = explode("-",$main_event_date_end);
	
	?>
	<tr>
		<td width="33%" class="top_txt"><?= AD_EVENT ?></td>
		<td width="18%" class="top_txt"><?= AD_CITY ?></td>
		<td width="21%" class="top_txt"><?= AD_VENUE ?></td>
		<td width="14%" class="top_txt"><?= AD_START_DATE ?></td>
		<td width="14%" class="top_txt"><?= AD_END_DATE ?></td>
	</tr>
	<tr>
		<td width="33%"><?php echo stripslashes($obj->f('event_name_en'));?></td>
		<td width="18%"><?php echo $obj->f('city_name');?></td>
		<td width="21%"><?php echo $obj->f('venue_name');?></td>
		<td width="14%"><?php echo $main_eve_day."/".$main_eve_mon."/".$main_eve_yr;?></td>
		<td width="14%"><?php echo $main_eve_day_end."/".$main_eve_mon_end."/".$main_eve_yr_end;?></td>
	</tr>
	</table>
	<br />
	...
	<tr>
        ...
        <td width="14%" class="top_txt" style="text-align:left"><?= AD_VENUE ?></td>
        <td width="14%" class="top_txt" style="text-align:left"><?= AD_EVENT_DATE ?></td>
	...
	if($num>0)
	{
		...
		{
			list($sub_event_date,$lain_event_time) = explode(" ",$objlist->f('event_start_date_time'));
			list($sub_eve_yr,$sub_eve_mon,$sub_eve_day) = explode("-",$sub_event_date);
			
			list($sub_event_date_end,$lain_event_time_end) = explode(" ",$objlist->f('event_end_date_time'));
			list($sub_eve_yr_end,$sub_eve_mon_end,$sub_eve_day_end) = explode("-",$sub_event_date_end);
			
?>
	<tr>	<?php /*?>onclick="EditSubEvent('<?php echo $objlist->f('event_id')?>','<?php echo $obj_parent->f('event_id');?>');"<?php */?>
	    <td><?php echo stripslashes($objlist->f('event_name_en'));?></td>
		<td><?php echo $objlist->f('venue_name');?></td>
		<td><?php echo $sub_eve_day."/".$sub_eve_mon."/".$sub_eve_yr;?></td>
		<td><?php echo date('g:i A',strtotime($lain_event_time))." - ".date('g:i A',strtotime($lain_event_time_end));?></td>
		<td><a href="<?php echo $obj_base_path->base_path(); ?>/admin/edit_sub_events_edit.php?sub_event_id=<?php echo $objlist->f('event_id')?>&event_id=<?php echo $obj_parent->f('event_id');?>"><img src="<?php echo $obj_base_path->base_path(); ?>/images/edit.gif" alt="" width="20" height="16" /></a></td>
		<td><a href="javascript:void(0);" onclick="deleteSubEvent('<?php echo $objlist->f('event_id')?>','<?php echo $obj_parent->f('event_id');?>');"><img src="<?php echo $obj_base_path->base_path(); ?>/images/cross.gif" alt="" width="20" height="16" /></a></td>
	</tr>
	    <?php
		}
	}
    ...
    <tr>
      ...
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
            ...
            <td style="color:#FF0000; padding: 0;"><strong>/</strong></td>
            <td style="color:#FF0000; padding: 0 0 0 13px;"><strong><?= AD_YEAR_FORMAT ?><?php //echo $st_eve_hr;?></strong></td>
        </tr>
        <tr>
        ...
        <tr>
		  ...
		  <select name="event_hr_st" class="selectbg" id="event_hr_st" title="Please select event hour" style="width:50px;float:left;" onChange="changeTime(this.value);">
            <?php //echo $st_eve_hr;
                  for($i=0; $i<13; $i++) {
                  ?>
            <option value="<?php echo $i; ?>" <?PHP if($i==$hr) {echo 'selected="selected"';}?>><?php echo $i; ?></option>
            <?php }?>
          </select></td>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
     <tr>
      ...
      <td width="22%">
		  ...
		  <select name="venue_county" id="venue_county" class="selectbg12"  onchange="getCity(this.value); saveAutoEvent()">
   ...
   <?php /*?> <textarea name="page_content_sp" cols="35" rows="10"><?php if($msg!="" && $page_content_sp!=""){echo $page_content_sp;}?></textarea><?php */?>
    ...
    <div style="float: left; margin: 0 auto;">	
		<span class="event_fieldbg8">
            ...
            <textarea name="short_desc_sp" id="short_desc_sp" class="event_field" style="width: 290px; margin: 5px 0; padding: 3px 5px; height: 60px;" onblur="saveAutoEvent();" onkeyup="limiter()"><?php echo $short_desc_sp;?></textarea>		
            <script type="text/javascript">
	...
	<div style="float: right; margin: 0 auto;">	
        <span class="event_fieldbg8">
            ...
            <textarea name="short_desc_en" id="short_desc_en" class="event_field" style="width: 290px; margin: 5px 0; padding: 3px 5px; height: 60px;" onblur="saveAutoEvent();" onkeyup="limiter1()"><?php echo $short_desc_en;?></textarea>
            <script type="text/javascript">
    ...
    <div>
     <ul>
		...
		<li><input name="radio_access" type="radio" value="2" onclick="showticketingsection(); saveAutoEvent();" <?php if($radio_access==2){ echo 'checked="checked"'; }?> /><strong><?= AD_TICKETING_RESERVATIONS_OP_THREE ?></strong></li>
        <li><input name="radio_access" type="radio" value="0" onclick="showticketingsection(); saveAutoEvent();" <?php if($radio_access==0){ echo 'checked="checked"'; }?> /><strong><?= AD_TICKETING_RESERVATIONS_OP_TWO ?> <?php "dd".$radio_access;?></strong></li>
...
function checkCat(category_id)
{
	...
	<div class="eventag_box">
		...
		<div class="clear"></div>
		<div>
        	<input type="text" name="event_tag" id="event_tag" value="<?php echo $event_tag;?>" class="textbg_add"/>
        	<input type="button" name="save" value="<?= AD_BUTTON_ADD ?>" class="btn_add" onblur="saveAutoEvent();" /></div>
		<div class="clear"></div>
