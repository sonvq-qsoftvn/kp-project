...
//mail("unified.subhrajyoti@gmail.com", "PAYPAL1", "","admin@gmail.com");
	...
	while (!feof($fp)) {
		...
		if (strcmp($res, "VERIFIED") == 0) 
		{
			...
			//user detail
			$tid = $obj->add_order($data['item_name'],$data['item_number'],$data['payment_status'],$data['payment_amount'],$data['payment_currency'],$data['txn_id'],$data['receiver_email'],$data['payer_email'],$data['event_id'],$data['ticket'],$data['payment'],$data['ticket_id'],$data['multi_id'],$data['user_id']);
			
			$_SESSION['tid'] = $tid;
			
			$obj->delete_order($data['user_id']);
