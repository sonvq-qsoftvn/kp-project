...
<meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1"   /> <!---"charset=iso-8859-1" for English, Spanish, French, German, etc.-->
<!--<meta property="og:locale" content="en_US" />-->
<meta property="og:locale" content="<?php if($_SESSION['langSessId']=='eng'){echo "en_US";}else{echo "es_ES";}?>" />
<meta property="og:type" content="website" />
<meta property='fb:app_id' content='149448255219243' />
<title>My Bookings Events</title>
         ...
         <div class="left_panel bg">
           <div class="cheese_box">
		    ...
		    <th width="15%">Status options</th>
                  ...
                  ?>
                  <tr>
                    <td><?php if($_SESSION['langSessId']=='eng') { echo '<a href="'.$obj_base_path->base_path().'/event/'.$obj_bookings_events->f('event_id').'">'.htmlentities(stripslashes($obj_bookings_events->f('event_name_en'))).'</a>'; } else { echo '<a href="'.$obj_base_path->base_path().'/event/'.$obj_bookings_events->f('event_id').'">'.$obj_bookings_events->f('event_name_sp').'</a>';}?>
		  <br/>
			...
			<a onclick="all_share('<?php echo $obj_bookings_events->f('event_id')?>','<?php echo $obj_base_path->base_path().'/event/'.$obj_bookings_events->f('event_id')?>','<?php if($_SESSION['langSessId']=='eng'){echo "en_US";}else{echo "es_ES";}?>','<?php echo $j?>')"><img src='<?php echo $obj_base_path->base_path(); ?>/images/share.png' width="28" height="27" border="0" alt="<?php if($_SESSION['langSessId']=='eng'){echo "Share";}else{echo "Comparte";}?>"/></a>&nbsp;&nbsp;
			  ...
			  <div style="display:none;">
			   ...
			   <div><span class="heading7">Emails:</span><span><textarea name="email" id="email_<?php echo $j?>" rows="5" cols="50" class="email"></textarea></span></div>
			<div><span class="heading7">Event's Details:</span></span><textarea  disabled name="body" id="body_<?php echo $j?>"  rows="10" cols="50">Hi,<?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($obj_bookings_events->f('event_name_en'))); } else { echo  $obj_bookings_events->f('event_name_sp');}?> is coming up on <?php if($_SESSION['langSessId']=='eng') { echo date("D",strtotime($obj_bookings_events->f('start')))." ".date("M",strtotime($obj_bookings_events->f('start')))." ".date("d",strtotime($obj_bookings_events->f('start'))).", ".date("Y",strtotime($obj_bookings_events->f('start')))." - ".date('g:i A',strtotime($obj_bookings_events->f('start'))); } else { setlocale(LC_TIME, 'es_ES'); echo  strftime("%a",strtotime($obj_bookings_events->f('start')))." ".strftime("%e",strtotime($obj_bookings_events->f('start')))." de ".strftime("%b",strtotime($obj_bookings_events->f('start'))).", ".strftime("%Y",strtotime($obj_bookings_events->f('start')))." ".strftime('%l:%M%p',strtotime($obj_bookings_events->f('start'))); } ?> at <?php if($_SESSION['langSessId']=='eng') { echo htmlentities(stripslashes($obj_bookings_events->f('venue_name')))." in ".htmlentities(stripslashes($obj_bookings_events->f('city_name'))); } else { echo $obj_bookings_events->f('venue_name_sp')." in ".$obj_bookings_events->f('city_name_sp'); }?> </textarea></span></div>
			   <div><span class="heading7">Message:</span><span><textarea name="message" id="message_<?php echo $j?>"  rows="10" cols="50" class="message"></textarea></span></div>
