...
$obj_venue_location->getStateCountyByEventID($event_id);
$obj_venue_location->next_record();
...
$event_end_date_time = $objEvent->f('event_end_date_time');
}
else
{
...
$event_start_date_time = $obj_chk->f('event_start_date_time');
$event_end_date_time = $obj_chk->f('event_end_date_time');
}
...
$new_state_name = preg_replace('#[^a-zA-Z0-9]#', ' ', trim($obj_venue_location->f('state_name')));
$new_county_name = preg_replace('#[^a-zA-Z0-9]#', ' ', trim($obj_venue_location->f('county_name')));
$new_city_name = preg_replace('#[^a-zA-Z0-9]#', ' ', trim($obj_venue_location->f('city_name')));

$new_event_name_en = preg_replace('/\s{2,}/',' ', $new_event_name_en);
$new_event_name_sp = preg_replace('/\s{2,}/',' ', $new_event_name_sp);
$new_state_name = preg_replace('/\s{2,}/',' ', $new_state_name);
$new_county_name = preg_replace('/\s{2,}/',' ', $new_county_name);
$new_city_name = preg_replace('/\s{2,}/',' ', $new_city_name);

$new_event_name_en = str_replace(' ', '-',strtolower($new_event_name_en));
$new_event_name_sp = str_replace(' ', '-',strtolower($new_event_name_sp));
$new_state_name = str_replace(' ', '-',strtolower($new_state_name));
$new_county_name = str_replace(' ', '-',strtolower($new_county_name));
$new_city_name = str_replace(' ', '-',strtolower($new_city_name));
...
$eventENRoute = $obj_base_path->base_path()."/en/event/" . $event_id . "/" . $new_state_name . "/" . $new_county_name . "/" . $new_city_name . "/" . $new_event_name_en;
$eventESRoute = $obj_base_path->base_path()."/es/evento/" . $event_id . "/" . $new_state_name . "/" . $new_county_name . "/" . $new_city_name . "/" . $new_event_name_sp;
...
if($_REQUEST['lang']=="")
{
	...
	if($_SESSION['langSessId']=='eng')
	{
        header("location: " . $eventENRoute);
		exit;
	}
	else{
		header("location: " . $eventESRoute);
		exit;
	}
...
if (!isset($_REQUEST['state']) || !isset($_REQUEST['county']) || !isset($_REQUEST['city'])) {

	if($_SESSION['langSessId']=='eng')
	{
		$_SESSION['event_state'] = $new_state_name;
		header("location: " . $eventENRoute);
		exit;
	}
	elseif($_SESSION['langSessId']=='spn'){ 
...
//================================  for meta====================   
   /*=====================EVENT DESCRIPTION==============================*/
	...
	//echo "k_t= ".$kp_event_title;
	
	//echo "k_D= ".$kp_event_desc;
...
?>
	...
	<title><?php  echo htmlentities(stripslashes($objEvent->f('event_name_en')));?></title>
        <meta name="title" content="<?php  echo $kp_event_title;?>" />
        <meta name="description" content="<?php echo $kp_event_title . ', ' . $kp_event_desc;?>" />
        
        <!-- Schema.org markup for Google+ -->
        <meta itemprop="name" content="<?php  echo htmlentities(stripslashes($objEvent->f('event_name_en')));?>" />
        <meta itemprop="description" content="<?php echo $kp_event_title . ', ' . $kp_event_desc;?>" />        	
        
        <meta name="keywords" content="<?php echo $objEvent->f('event_name_en') . ', ' . $obj_venue->f('venue_name') . 
                ', ' . $obj_venue_location->f('state_name') . ', ' . $obj_venue_location->f('county_name') . ', ' . $obj_venue_location->f('city_name'); ?>">
...
?>
        <meta http-equiv="Content-Type" content="text/html;charset=utf-8"   />
	<title><?php  echo htmlentities(stripslashes($objEvent->f('event_name_sp')));?></title>
        <meta name="title" content="<?php  echo $kp_event_title;?>" />
        <meta name="description" content="<?php setlocale(LC_TIME, 'es_ES.UTF-8'); echo $kp_event_title . ', ' . strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');?>" />        
        
        <!-- Schema.org markup for Google+ -->
        <meta itemprop="name" content="<?php  echo htmlentities(stripslashes($objEvent->f('event_name_sp')));?>" />
        <meta itemprop="description" content="<?php setlocale(LC_TIME, 'es_ES.UTF-8'); echo $kp_event_title . ', ' . strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');?>" />        
    
    
        <meta name="keywords" content="<?php echo $objEvent->f('event_name_sp') . ', ' . $obj_venue->f('venue_name_sp') . 
                ', ' . $obj_venue_location->f('state_name') . ', ' . $obj_venue_location->f('county_name') . ', ' . $obj_venue_location->f('city_name'); ?>">
	...
	<meta property="og:title" content="<?php  echo $kp_event_title;?>" />	
	<meta property="og:url" content="<?php echo $actual_link; ?>" />	
	<meta property="og:description" content="<?php setlocale(LC_TIME, 'es_ES.UTF-8'); echo strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date)).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".htmlentities($obj_venue->f('st_name')).", ".htmlentities($objEvent->f('event_short_desc_sp'));?>" />	
<?php
}
?>
...
<meta itemprop="image" content="<?php echo $obj_base_path->base_path(); ?>/images/kpassa_logo_fb.png" />
<?php
}
else
{
...
<meta itemprop="image" content="<?php echo $obj_base_path->base_path(); ?>/files/event/large/<?php echo $objEvent->f('event_photo');?>"/>
<?php
}
?> 
...
<?php if($objEvent->f('event_photo')=='')
{
$tweet_image = $obj_base_path->base_path()."/images/kpassa_logo_fb.png";
}
else
{
$tweet_image =  $obj_base_path->base_path()."/files/event/large/".$objEvent->f('event_photo');
}
//echo $tweet_image;
?>

<meta name="twitter:image:src" content="<?php echo $tweet_image;?>">
<!----------------------------->

<link rel="alternate" href="<?php echo $eventENRoute; ?>" hreflang="en" />
<link rel="alternate" href="<?php echo $eventESRoute; ?>" hreflang="es" />
...
<div id="maindiv">	
	...
	<div class="body_bg">    	
        ...
        <?php 
            ...
            $objEventById->getSchemaTypeByEventId($event_id); 
        ...
        ?>
        <div class="container" itemscope itemtype="<?php 
        ...
        ?>">
            <?php 
                if(count($arraySchemaType) > 1) {
                    for($i = 1; $i < count($arraySchemaType); $i++) {
                        echo '<meta itemprop="additionalType" href="http://schema.org/' . $arraySchemaType[$i]. '" />';
                    }
                }
            ?>
            <meta itemprop="url" content="<?php echo "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"; ?>" />
            ...
            <div class="clear"></div>
            	...
            	<div class="cheese_box">
               	<h1 class="heading1" itemprop="name">
                    <?php
                    if ($sub_id == '') {
                        if ($_SESSION['langSessId'] == 'eng') {
                            echo htmlentities(substr(stripslashes($objEvent->f('event_name_en')), 0, 60));
                        } else {
                            echo htmlentities(substr(stripslashes($objEvent->f('event_name_sp')), 0, 60));
                        }
                    } else {
                        if ($_SESSION['langSessId'] == 'eng') {
                            echo htmlentities(substr(stripslashes($obj_chk->f('event_name_en')), 0, 60));
                        } else {
                            echo htmlentities(substr(stripslashes($obj_chk->f('event_name_sp')), 0, 60));
                        }
                    }
                    ?>
					...
					?>
					 ...
					 ?>       
                            ...
                            <td style="height: auto;">
								...
								<?php if($_SESSION['langSessId']=='eng') echo $objmul_date->f('event_name_en'); else echo htmlentities(stripslashes($objmul_date->f('event_name_sp')));?> <br/><?php echo date('g:i A',strtotime($time))." - ".date('g:i A',strtotime($end_time)); ?>
								</a>
								<script type="text/javascript">
									$(document).ready(function() {
										$(".link").fancybox({ 
            ...
            <?php if($objmulti_event->num_rows()){ ?>                        
                <div id='carousel_container'>
					...
					?></th>
						...
						else { setlocale(LC_TIME, 'es_ES.UTF-8');  echo  strftime('%l:%M%p',strtotime($multi_event_time_new));}?>
				...
				?>
					 ...
					 ?>                      
                       <?php if($_SESSION['langSessId']=='eng'){?>
		       <div class="timetxt">
                   <span><?php echo date("D",strtotime($obj_cur_eve_dt->f('multi_start_time')))." ".date("M",strtotime($obj_cur_eve_dt->f('multi_start_time')))." ".date("d",strtotime($obj_cur_eve_dt->f('multi_start_time'))).", ".date("Y",strtotime($obj_cur_eve_dt->f('multi_start_time')));?> - <?php echo date('g:i A',strtotime($obj_cur_eve_dt->f('multi_start_time'))); ?></span>
                   <meta itemprop="startDate" content="<?php echo date("Y-m-d H:i", strtotime($obj_cur_eve_dt->f('multi_start_time'))); ?>" />
                   to
                   <span><?php echo date('g:i A',strtotime($obj_cur_eve_dt->f('multi_end_time'))); ?></span>
                   <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($obj_cur_eve_dt->f('multi_end_time'))); ?>" />
                       </div>
		       <?php }
		       else{
			setlocale(LC_TIME, 'es_ES.UTF-8');
			?>
		       <div class="timetxt">
                   <span><?php echo strftime("%a",strtotime($obj_cur_eve_dt->f('multi_start_time')))." ".strftime("%e",strtotime($obj_cur_eve_dt->f('multi_start_time')))." de ".strftime("%b",strtotime($obj_cur_eve_dt->f('multi_start_time'))).", ".strftime("%Y",strtotime($obj_cur_eve_dt->f('multi_start_time')));?> - <?php echo strftime('%l:%M%p',strtotime($obj_cur_eve_dt->f('multi_start_time'))); ?></span>
                   <meta itemprop="startDate" content="<?php echo date("Y-m-d H:i", strtotime($obj_cur_eve_dt->f('multi_start_time'))); ?>" />
                   to
                   <span><?php echo strftime('%l:%M%p',strtotime($obj_cur_eve_dt->f('multi_end_time'))); ?></span>
                   <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($obj_cur_eve_dt->f('multi_end_time'))); ?>" />
                       </div>
			<?php }?>
                       <?php
			...
			<?php if($_SESSION['langSessId']=='eng'){?>
                       <div class="timetxt">
                           <span><?php echo date("D",strtotime($event_date))." ".date("M",strtotime($event_date))." ".date("d",strtotime($event_date)).", ".date("Y",strtotime($event_date));?> - <?php echo date('g:i A',strtotime($event_time)); ?></span>
                           <meta itemprop="startDate" content="<?php echo date("Y-m-d H:i", strtotime($event_start_date_time)); ?>" />
                           to
                           <span><?php echo date('g:i A',strtotime($event_time_end)); ?></span>
                           <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                       </div>
		       <?php }
		       else{ setlocale(LC_TIME, 'es_ES.UTF-8');
		       ?>
		       <div class="timetxt">
                   <span><?php echo  strftime("%a",strtotime($event_date))." ".strftime("%e",strtotime($event_date))." de ".strftime("%b",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date));?> - <?php echo strftime('%l:%M%p',strtotime($event_time)); ?></span> 
                   <meta itemprop="startDate" content="<?php echo date("Y-m-d H:i", strtotime($event_start_date_time)); ?>" />
                   to
                   <span><?php echo strftime('%l:%M%p',strtotime($event_time_end)); ?></span>
                   <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                       </div>
		       <?php }?>
                        <?php
		       ...
		       <?php if($_SESSION['langSessId']=='eng'){?>
                       <div class="timetxt">
						...
						{ 
							echo " <br /> To "; ?> 
                            <span><?php echo  date("D",strtotime($event_date_end))." ".date("M",strtotime($event_date_end))." ".date("d",strtotime($event_date_end)).", ".date("Y",strtotime($event_date_end))." - ";?> <?php echo date('g:i A',strtotime($event_time_end)); ?> </span>
                            <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                            <?php
						} 
						else 
						{ 
						?>
                            to <span><?php echo date('g:i A',strtotime($event_time_end)); ?></span>
                            <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                            <?php } ?>
                       </div>
		       <?php }
		       ...
		       <div class="timetxt">
                   ...
                   <?php
						...
						{ 
							echo " <br /> To "; ?>
                            <span><?php echo  strftime("%a",strtotime($event_date_end))." ".strftime("%e",strtotime($event_date_end))." de ".strftime("%b",strtotime($event_date_end)).", ".strftime("%Y",strtotime($event_date_end))." - ";?> <?php echo strftime('%l:%M%p',strtotime($event_time_end)); ?></span>
                            <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                            <?php
						} 
						else 
						{ 
						?>
                            to <span><?php echo strftime('%l:%M%p',strtotime($event_time_end)); ?></span>
                            <meta itemprop="endDate" content="<?php echo date("Y-m-d H:i", strtotime($event_end_date_time)); ?>" />
                            <?php } ?>
                       </div>
		       <?php }?>
...
-->                        </div>
                        ...
                        <?php 
                            if($sub_id != ''){
                                    if($_SESSION['langSessId']=='eng') echo '<span itemprop="description">' . htmlentities($obj_chk->f('event_short_desc_en')); else echo htmlentities($obj_chk->f('event_short_desc_sp')) . '</span>';
                            }
                            else{
                                    if($_SESSION['langSessId']=='eng') echo '<span itemprop="description">' . htmlentities($objEvent->f('event_short_desc_en')); else echo  htmlentities($objEvent->f('event_short_desc_sp')) . '</span>';
                            }
                        ?>
			...
			<div class="btn2_sudip"><a onclick="scroll_to();"><?php if($_SESSION['langSessId']=='eng'){?> More <?php }else{?> Mas <?php }?></a></div>
                        ...
                        <?php 
                            ...
                            if($objTypesEvent->num_rows())
                            {
                                ...
                                while($objTypesEvent->next_record()) {
                                    ...
                                    if ($_SESSION['langSessId']=='eng') {
                                        echo $objTypesEvent->f('event_types');
                                    } else {
                                        echo $objTypesEvent->f('event_types_sp');
                                    }
                                    $count++;
                                }
                                echo '</p>';
                        ...
                        <?php 
                            $objEventById->getCategoryByEventId($event_id); 
                            if ($objEventById->num_rows() > 0) {
                                while($objEventById->next_record()){				
                                    echo '<p>';
                                    if ($_SESSION['langSessId']=='eng') {
                                    ...
                                    $objSubEventById->getSubCategoryByCategoryIdAndEventId($event_id, $objEventById->f('category_id'));
			    ...
			    if($sub_id == ''){
    			    ?>
                <div style="margin:10px 0 20px; font-weight:bold;" itemprop="location" itemscope itemtype="http://schema.org/Place">
                    ...
                    <div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
                        <div itemprop="name">
                            ...
                            ?>                                                                                            
                            <h2 style="margin: 0px 0px; padding: 0px 4px;">
                                <a style="color:#056A86" href="<?php echo $obj_base_path->base_path(). $objCommon->getCleanVenueURL($obj_venue->f('venue_id'), $objVenueLocation, $venueLang); ?>">
                                    <?php echo htmlentities($obj_venue->f('venue_name')); ?>
                                </a>
                            </h2>
                            <p style="margin: 0px 0px; padding: 0px 4px; font-weight:normal;"><span itemprop="streetAddress"><?php echo htmlentities($obj_venue->f('venue_address')); ?></span>
                                <br />
                                <?php echo '<span itemprop="addressLocality">' . htmlentities($obj_venue->f('city')) . '</span>, <span itemprop="addressRegion">' . htmlentities($obj_venue->f('st_name')) . '</span>'; ?></p>
                        </div>
                    </div>
                    <meta itemprop="name" content="
                        <?php echo htmlentities($obj_venue->f('venue_name')) . ', ' . htmlentities($obj_venue->f('venue_address')); ?> 
                        <?php echo htmlentities($obj_venue->f('city')) . ', ' . htmlentities($obj_venue->f('st_name')); ?>" />
                </div>
			...
			else
			{
				...
				<div style="margin:10px 0 20px; font-weight:bold;" itemprop="location" itemscope itemtype="http://schema.org/Place">
                    ...
                    <div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
                        <div itemprop="name">
                            <?php 
                                $objVenueLocation->getVenueLocationByVenueID($obj_venue_sub->f('venue_id'));
                                $objVenueLocation->next_record();
                                if($_SESSION['langSessId']=='eng'){
                                    $venueLang = 'en';
                                } else {
                            ...
                            ?>    
                            <h2 style="margin: 0px 0px; padding: 0px 4px;">
                                <a style="color:#056A86" href="<?php echo $obj_base_path->base_path(). $objCommon->getCleanVenueURL($obj_venue_sub->f('venue_id'), $objVenueLocation, $venueLang); ?>">
                                    <?php echo htmlentities($obj_venue_sub->f('venue_name'));?>
                                </a>
                            </h2>
                            <p style="margin: 0px 0px; padding: 0px 4px; font-weight:normal;"><span itemprop="streetAddress"><?php echo $obj_venue_sub->f('venue_address');?></span>
                            <br />
                            <?php echo '<span itemprop="addressLocality">'.htmlentities($obj_venue_sub->f('city')).'</span>, <span itemprop="addressRegion">'.htmlentities($obj_venue_sub->f('st_name')).'</span>';?></p>
                        </div>
                    </div>
                    <meta itemprop="name" content="
                        <?php echo htmlentities($obj_venue->f('venue_name')) . ', ' . htmlentities($obj_venue->f('venue_address')); ?> 
				...
				<?php  $objfeatureimage->isfeatureImage($event_id);
					...
					<li style="margin: 0; display: block;">
                                <a href="#feature_image" id="feature">
                                	<img itemprop="image" src="<?php echo $obj_base_path->base_path(); ?>/files/event/medium/<?php echo htmlentities(stripslashes($objfeatureimage->f('media_url')));?>"  border="0"  /></a>
                                </li>
...
-->                <div class="clear"></div>
        ...
        <?php if($obj_event_media->num_rows()){ ?> <!---if images----------------------->
			...
			<div class="heading"><?php if($_SESSION['langSessId']=='eng') { echo "Media Gallery" ; } else { echo htmlentities("Galería multimedia") ;} ?></div>
		...
		//echo "<br/>var=".$var;?>
	     ...
	     <?php if($_SESSION['langSessId']=='eng') { ?> 
	       <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;" id="media_click"><a href='<?php echo $obj_base_path->base_path(); ?>/en/event/<?php echo htmlentities(stripslashes($objEvent->f('event_name_en')));?>/gallery/<?php echo $media_id;?>/<?php echo html_entity_decode($media_name);?>' class="fancybox"><img src="<?php echo $obj_base_path->base_path(); ?>/files/event/thumb/<?php echo htmlentities(stripslashes($obj_event_media->f('media_url'))); ?>" alt="" width="190" height="143" /></a></li>
	       <?php } else {?>
	       <li style="border: 1px solid #CCCCCC; margin: 0 4px; width: 170px;" id="media_click"><a href='<?php echo $obj_base_path->base_path(); ?>/es/evento/<?php echo htmlentities(stripslashes($objEvent->f('event_name_sp')));?>/galeria/<?php echo $media_id;?>/<?php echo html_entity_decode($media_name);?>' class="fancybox"><img src="<?php echo $obj_base_path->base_path(); ?>/files/event/thumb/<?php echo htmlentities(stripslashes($obj_event_media->f('media_url'))); ?>" alt="" width="190" height="143" /></a></li> <?php } ?>
	       <?php } // if not video
