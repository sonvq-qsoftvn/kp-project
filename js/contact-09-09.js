...
$(document).ready( function() {
	...
	$('#submit1').click( function() {
		if( $('#contact').valid() ) {
			...
			//setTimeout("window.location=window.location.protocol+'//'+window.location.host+'/'+'webroot/team5/kcpasa/registration'",1000)
		}
		else {
...
function ajaxSubmit() {
	...
	var data = 'fname=' +fname+ '&lname=' +lname+ '&email=' +email+ '&phone=' +phone+ '&country_id=' +country_id+ '&country_code=' +country_code+ '&password=' +password+ '&account_type=' +account_type+ '&country_id=' +country_id+ '&country_code=' +country_code;
		
	$.ajax({
		url: window.location.protocol+'//'+window.location.host+'/'+'kcpasa/registration_process.php',
		type: 'POST',
		dataType: 'json',
		data: data,
		cache: false,
