...
function tkt_num(count,cart_id,event_id,payment,promotion){
	sendData = {"count":$("#ticket_num"+count).val(),"cart_id":cart_id,"event_id":event_id,"payment":payment,"promotion":promotion};
	//alert($("#amount").val());
	$("#loader").show();
	 $.ajax({ 
...
<?php if($_SESSION['langSessId']=='eng') {?>  US$ <?php }elseif($_SESSION['langSessId']=='spn'){?>EE.UU. $<?php }?> <input type="radio" name="cur" onclick="change(<?php echo $e_id;?>,'us');" id="" value="" <?php if($_SESSION['pay'] == 'us'){ echo "checked";}?> />
			  ...
			  ?>
                        ...
                        <tr>
                          <td>
			    <select name="ticket_num" id="ticket_num<?php echo $count;?>" onChange="tkt_num(<?php echo $count;?>,<?php echo $obj_ticket->f('cart_id');?>,<?php echo $e_id;?>,<?php echo $objEvent->f('include_payment');?>,<?php echo $objEvent->f('include_promotion');?>);">
	...
	<div class="clear"></div>		 
	 <div class="account_box">
	   ...
	   <div class="blue_buy">
		...
		<li><a href="#"><?php if($_SESSION['langSessId']=='eng') {?>Credit/Debit Card<?php }elseif($_SESSION['langSessId']=='spn'){?> Tarjeta de Credito/Debito<?php }?></a><br/><img src='<?php echo $obj_base_path->base_path(); ?>/images/logo5.gif' width="35" height="19" border="0"/><br/><input name="radiobutton" id="tns" type="radio" value="radiobutton" onClick="pay_type('tns');"/></li>
