...
function new_signup(){
	...
	if (con_pass == '') {
		alert("Confirn Password cannot be blank");
		err = 1;
		//$("#con_password").val('');
		$("#con_password").focus();
		return false;
	...
	//sendData = {"country_id":$('#country_id').val()};
	 $.ajax({ 
	   url: "<?php echo $obj_base_path->base_path(); ?>/ajax_folder/ajax_set_password.php",
	   cache: false,
	   type: "POST",
	   data: $("#new_frm").serialize(),   
	   success: function(data){
	   //$("#country_code").val(data);
	   setTimeout("window.location=window.location.protocol+'//'+window.location.host+'/'+'webroot/team5/kpasapp/thankyou'",100);
	   }
	 });
			...
			<?php
				...
				if($pass_exists == ''){
					...
					<table width="100%" align="center" border="0" cellpadding="4" cellspacing="4" style="border-collapse:separate;">
					  <tr>
					    <td style="padding-left: 18px;"><?php echo PASS;?></td>
