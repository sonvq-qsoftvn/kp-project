...
function manager_captcha_reload()
		...
		function manager_captcha_reload_new(param)
		{
			if (param==2) {
				$.ajax({
					...
					success:function(msg)
					{
					 ...
					 //alert(msg);
					 $('#booking_captcha_old').val(msg);
					  $.get('<?php echo BASE_URL;?>registerclinicmanager', function() {
					   $('#booking_captca_img_val').attr('src', '<?php echo BASE_URL;?>app/webroot/next.php?text='+msg);
					  });
					}
				   });
			}
	...
	if (e == 0)
	{
		...
		$.ajax({
			...
			success:function(result)
			{
				...
				//$('.addanappointment_close btnclose').trigger();
			}
		    ...
		    <!---- This is for arrow to change the calender date for 7 days before  as well as 7 days after end---->
					...
					for($i=0;$i<288;$i++)
					{
						...
						for($j=0;$j<=6;$j++)
						{
							...
							{
								?>
							<td class="coll" id="<?php echo $count; ?>" rowspan="<?php echo $dc[$j];?>"><a href="javascript:void(0)" class="initialism addanappointment_open icon1" onclick="booking_appointment('<?php echo $i;?>','<?php echo $all_dates[$j];?>','<?php echo $start;?>','<?php echo $dc[$j];?>',<?php echo $count; ?>)" ><?php echo "<img src='".BASE_URL."frontend/images/manageinfo_icon1.png'>";?></a><a href="javascript:void(0)" class="icon2"  onclick="blockException('<?php echo $i;?>','<?php echo $all_dates[$j];?>','<?php echo $clinic_id ?>')" ><?php echo "<img src='".BASE_URL."frontend/images/manageinfo_icon2.png'>";?></a></td>
...
<div id="addanappointment" class="well">
	...
	<br>
	<a href="javascript:void(0);" onclick="manager_captcha_reload_new(2);">Reload</a>
	</div>
	<?php $manager_cap_val=$_SESSION['captcha_val'];
	echo $this->Form->input('user_captcha',array('label' => FALSE, 'div' => FALSE, 'type'=>'hidden', 'placeholder'=>'Enter captcha value','class' =>'user-in', 'id'=>'booking_captcha_old','value'=>$manager_cap_val));
	?>
	<div class="code-in"><input name="booking_captcha" placeholder="Enter captcha value" class="user-in" id="booking_captcha" onblur="captcha_check()" type="text" /><span id="msg_new" class="error"></span></div>
...
<div id="rescheduleappointment" class="well">
	<div class="fade_cont">
		...
		<?php $manager_cap_val=$_SESSION['captcha_val']; ?>
		<div class="code-in"><input name="booking_captcha" placeholder="Enter captcha value" class="user-in" id="booking_recaptcha" onblur="captcha_check('<?php echo $manager_cap_val; ?>')" type="text" /><span id="msg_new_reschedule" class="error"></span></div>
