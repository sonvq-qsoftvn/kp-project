...
<script>
	$(document).ready(function(){
		...
		$('#manager_reg_btn').click(function(){	
			$("#clinic_mang_reg").validate({
				...
				submitHandler: function(form) {
					...
					var sub_catcha_val=$('#manager_captcha').val();
					var sess_cap_val=$('#manager_captcha_old').val();
					if (sub_catcha_val != '' && sub_catcha_val != sess_cap_val)
					{
		...
		captcha_reload(1);
		manager_captcha_reload(1);
		
		//-------------------captcha reload function------------    
	});
	function captcha_reload(param)
	{
		if (param==2) {
				$.ajax({
					url:'<?php echo BASE_URL;?>appointments/reload_captcha',
					type:'post',
					success:function(msg)
					{
					
					 //alert(msg);
					 $('#user_captcha_old').val(msg);
					  $.get('<?php echo BASE_URL;?>registerclinicmanager', function() {
					   $('#captca_img_val').attr('src', '<?php echo BASE_URL;?>app/webroot/next.php?text='+msg);
					  });
					}
				   });
			}
	...
	function manager_captcha_reload(param)
	{
		if (param==2) {
				$.ajax({
					url:'<?php echo BASE_URL;?>appointments/reload_captcha',
					...
					success:function(msg)
					{
					 ...
					 //alert(msg);
					 $('#manager_captcha_old').val(msg);
					  $.get('<?php echo BASE_URL;?>registerclinicmanager', function() {
					   $('#manager_captca_img_val').attr('src', '<?php echo BASE_URL;?>app/webroot/next.php?text='+msg);
					  });
					}
				   });
			}
	...
	{	
		...
		var sess_cap_val=document.getElementById('user_captcha_old').value;
		if (sub_catcha_val != '' && sub_catcha_val != sess_cap_val)
		{
		    $("#msg").html("<i style='color:red; border: 1px solid red;'>Wrong Captcha</i>");
	...
	function manager_captcha_check()
	{
		var sub_catcha_val=document.getElementById('manager_captcha').value;
		var sess_cap_val=document.getElementById('manager_captcha_old').value;
		if (sub_catcha_val != '' && sub_catcha_val != sess_cap_val)
		{
		    $("#msg_captcha").html("<i style='color:red; border: 1px solid red;'>Wrong Captcha</i>");
	...
	<input <?php if(!strpos($user_agent,"Mac OS")){?>id="datepicker" type="text" <?php } else{?> type="date" style='height:38px;'<?php }?>   value="<?php echo date('m/d/Y', strtotime($dob)); ?>" name='date_of_birth' placeholder="Date of Birth " class="user-in-date">
							...
							<div class="form-row">
								<div class="col-md-5">
									<div class="code-b">
										...
										<a href="javascript:void(0);" onclick="captcha_reload(2);">
									...
									<div class="code-in">
										<?php
											...
											echo $this->Form->input('user_captcha',array('label' => FALSE, 'div' => FALSE, 'type'=>'hidden', 'placeholder'=>'Enter captcha value','class' =>'user-in', 'id'=>'user_captcha_old','value'=>$cap_val));
										?>
										<input name="user_captcha" placeholder="Enter captcha value" class="user-in" id="user_captcha" onblur="captcha_check()" type="text">
									</div>
									<div id='msg'></div>
								</div>
							...
							<div class="user-mnass">
								<div>
									...
									I agree to the 
									<?php echo $this->Html->link($terms_cond_reg, array('controller' => 'contents', 'action' => 'showcontent', 'alias' => 'terms-of-use'),array('target' => '_blank')); ?>
								</div>
					...
					<div id='clinic_manager_form'>
						<form action='<?php echo BASE_URL;?>registerclinicmanager' name='clinic_mang_reg' id='clinic_mang_reg' method='post' enctype='multipart/form-data'>
							...
							<div class="form-row">
								<div class="col-md-5">
									<div class="code-b">
										<?php echo $this->Html->image('../frontend/images/code.png',array('alt'=>'', 'id' => 'manager_captca_img_val')); ?>
										<span class="re-mic" onclick="manager_captcha_reload(2);" style="display: block;">
									...
									<div class="code-in">
										<?php
											...
											echo $this->Form->input('user_captcha',array('label' => FALSE, 'div' => FALSE, 'type'=>'hidden', 'placeholder'=>'Enter captcha value','class' =>'user-in', 'id'=>'manager_captcha_old','value'=>$manager_cap_val));
										?>
										<input name="manager_captcha" placeholder="Enter captcha value" class="user-in" id="manager_captcha" onblur="manager_captcha_check()" type="text" />
									</div>
