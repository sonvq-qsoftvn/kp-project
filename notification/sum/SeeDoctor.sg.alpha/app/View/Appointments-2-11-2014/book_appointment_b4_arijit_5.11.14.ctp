				...
				<div class="manage_appointment_form">
					<form name="book_appointment" id="book_appointment" action="" method="post">
						...
						<div class="clinic_name">
							<h2><span>Clinic Name:</span></h2>
							<div class="col-1">
								<input type="text" placeholder="Appointment Date" value="<?php echo $req_date; ?>" id="clinic_cal" class="datepicker user-in-date" onchange="ajax_clinic_timing(this.value);" />
								<label id="date_msg" style="font-weight: normal;"></label>
							</div>
							<div class="col-2">
								...
								<div class="upcoming_past">
									<button type="button" <?php if($cal_type == 'month')echo 'class="active"'; ?>  onclick="window.location='<?php echo BASE_URL.'appointments/book_appointment/cal:month' ?>'">Monthly</button>
									<button type="button" <?php if($cal_type == 'week')echo 'class="active"'; ?> onclick="window.location='<?php echo BASE_URL.'appointments/book_appointment/cal:week' ?>'">Weekly</button>
								</div>
							</div>    
						</div>	
					</form>
