...
<script type="text/javascript">
 ...
 function send_message_all()
 {
       ...
       if (user_name=='')
       {
	$('#show_message_usename_empty').show(); 
        $("#form_username").focus();
	return false;
       }
       else
       {
	 $('#show_message_usename_empty').hide(); 
       }
       if (subject=='')
       {
	$('#show_message_subject').show(); 
        $("#subject").focus();
	return false;
       }
       else
       {
	 $('#show_message_subject').hide(); 
       }
       if (message=='')
       {
	$('#show_message_body').show(); 
        $("#message").focus();
	return false;
       }
       else
       {
	 $('#show_message_body').hide(); 
       }
		...
		<!------For Send Message Start----->
                	<div class="msg_list_in">
                    	<div class="left" style='border-radius:0% 0% 0% 0%'>
							...
							?>
                            <h4><?php echo $logged_user_details['username'];?></h4>
						</div>
			<!----NA---->
                        <div class="middle">
			...
			<?php if(isset($to_user_details['username']))
			{
				$send_user = $to_user_details['username'];
				$disable ='disabled';
			}
			else
			{
...
<input type="text" name="form_username" id="form_username" class="subject_text" value= "<?php echo $send_user;?>" <?php echo $disable;?>>
			
			<span id="show_message_usename" style="display:none; color: #FF0000">Username is not exist. </span>
			<span id="show_message_usename_empty" style="display:none; color: #FF0000">Please enter user name. </span>
<!--                                <input type="text" name="username" id="username" class="typeahead subject_text  tt-input" >-->
			...
			<input type="text" name="subject" id="subject" class="subject_text">
			 <span id="show_message_subject" style="display:none; color: #FF0000">Please enter subject. </span>
			<p>Message</p>
			<textarea rows="5" cols="5" name="message" id="message" class="text_msg"></textarea><br>
			<span id="show_message_body" style="display:none; color: #FF0000">Please enter your message. </span><br>
...
if(trim($last_delemeter) == "thismsg")
{
	...
	if(count($all_messages))
	{
		...
		//echo "userid2= ".$to_user_details['id'];
		...
		?>
			<div class="msg_list_out">
				<div class="right" style='border-radius:0% 0% 0% 0%'>
						...
						<?php
						//pr($value);
						if(isset($to_user_details['profile_image']) && ($to_user_details['profile_image']!='') && (file_exists("frontend/uploads/profile_image/".$value['User']['profile_image'])))
						{
						?>
