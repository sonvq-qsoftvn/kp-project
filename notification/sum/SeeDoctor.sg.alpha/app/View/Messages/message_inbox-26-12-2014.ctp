             ...
             <table width="100%" border="0" cellspacing="0" cellpadding="0" class="msg_info">
              ...
              <?php if(!empty($all_messages_inbox)) 
                ...
                <tr class="msg_info_cont">
                    <td class="col-1"><div class="picdetails"><b style='color:#522900;'><?php if($all_messages_inboxs['Messagecontent']['fromtype']=='superadmin'){echo ($from_users[$all_messages_inboxs['Messagecontent']['fromid']]['Admin']['admin_email']); ?></b><br/><font style='color:#CC5252'><?php echo ucfirst('superadmin');}else{ $all_types=array('User','Clinic Manager');echo ($from_users[$all_messages_inboxs['Messagecontent']['fromid']]['User']['email']); ?></b><br/><font style='color:#CC5252'><?php echo ucfirst($all_types[$from_users[$all_messages_inboxs['Messagecontent']['fromid']]['User']['user_type']-1]); }?></font><br><font style='color:#003300'><?php $ret_str=($this->Functions->calculate_time_gap(date('Y-m-d H:i:s'),$all_messages_inboxs['Messagecontent']['datesent']));if($ret_str['Y']!=0||$ret_str['M']!=0){echo "Nearly ";} if($ret_str['Y']!=0){ echo $ret_str['Y'].' Year'; if($ret_str['Y']>1){echo 's';}}if($ret_str['M']!=0){ echo ' '.$ret_str['M'].' Month'; if($ret_str['M']>1){echo 's';}}if($ret_str['d']!=0){ echo ' '.$ret_str['d'].' Day'; if($ret_str['d']>1){echo 's ';}}if($ret_str['h']!=0){ echo ' '.$ret_str['h'].' Hour'; if($ret_str['h']>1){echo 's ';}}if($ret_str['m']!=0){ echo ' '.$ret_str['m'].' Minute'; if($ret_str['m']>1){echo 's ';}}if($ret_str['Y']!=0||$ret_str['M']!=0||$ret_str['d']!=0||$ret_str['h']!=0||$ret_str['m']!=0){echo ' ago ';}else{echo 'Just Now';}?></font></div></td>
                    
                    
                    <td class="col-2"><a href="<?php echo BASE_URL;?>messages/conversation/<?php echo $all_messages_inboxs['Messagecontent']['fromid'];?>/<?php echo $all_messages_inboxs['Messagecontent']['fromtype'];?>/<?php echo $all_messages_inboxs['Message']['id'];?>"><?php echo $all_messages_inboxs['Messagecontent']['subject'] ?></a></td>
                    <td class="col-3">
                    ...
                    <td class="col-5">
			<span>
			    <a href="<?php echo BASE_URL;?>messages/conversation/<?php echo $all_messages_inboxs['Messagecontent']['fromid'];?>/<?php echo $all_messages_inboxs['Messagecontent']['fromtype'];?>/<?php echo $all_messages_inboxs['Message']['id'];?>"><?php echo $this->Html->image('../frontend/images/icon35.jpg',array('alt'=>'')); ?></a>
                         </span>
			<!--<span><a href="#">
