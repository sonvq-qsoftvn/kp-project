...
<section class="emai-registration">
    <div class="topheading-box">
                ...
                <tr class="msg_info_cont">
                    <td class="col-1">
                        ...
                        <div class="pic">
                          ...
                          <?php
                                        if((isset($all_messages_outbox['ToUser']['profile_image']) &&  $all_messages_outbox['ToUser']['profile_image']!='') && (file_exists("frontend/uploads/profile_image/".$all_messages_outbox['ToUser']['profile_image'])))
                                        {
                                        ?>
                                                <img  width="51" height="50" src="<?php echo $this->webroot; ?>frontend/uploads/profile_image/thumbimage/<?php echo $all_messages_outbox['ToUser']['profile_image']; ?>" style="margin-bottom: 10px;">
                                        <?php
                                        }
                                        else
                                        {
                        ...
                        <div class="picdetails">
                                ...
                                if($all_messages_outbox['Message']['totype']=='superadmin'){
                                    
                                    //echo ($to_users['admin'][$all_messages_outbox['Message']['toid']]['Admin']['admin_email']); ?>
              <?php 
              
                               //echo ucfirst('superadmin');
              
                            echo ucfirst($all_messages_outbox['ToUser']['username']); 
      ...
      // echo ucfirst($all_types[$to_users['user'][$all_messages_outbox['Message']['toid']]['User']['user_type']-1]); 
       ...
       ?>
                                 <br/>
                                        ...
                                        <?php $ret_str=($this->Functions->calculate_time_gap(date('Y-m-d H:i:s'),$all_messages_outbox['Messagecontent']['datesent']));if($ret_str['Y']!=0||$ret_str['M']!=0){echo "Nearly ";} if($ret_str['Y']!=0){ echo $ret_str['Y'].' Year'; if($ret_str['Y']>1){echo 's';}}if($ret_str['M']!=0){ echo ' '.$ret_str['M'].' Month'; if($ret_str['M']>1){echo 's';}}if($ret_str['d']!=0){ echo ' '.$ret_str['d'].' Day'; if($ret_str['d']>1){echo 's ';}}if($ret_str['h']!=0){ echo ' '.$ret_str['h'].' Hour'; if($ret_str['h']>1){echo 's ';}}if($ret_str['m']!=0){ echo ' '.$ret_str['m'].' Minute'; if($ret_str['m']>1){echo 's ';}}if($ret_str['Y']!=0||$ret_str['M']!=0||$ret_str['d']!=0||$ret_str['h']!=0||$ret_str['m']!=0){echo ' ago ';}else{echo 'Just Now';}?></div></td>
                    
                    
                    <td class="col-2"><a href="<?php echo BASE_URL;?>messages/conversation/<?php echo $all_messages_outbox['Message']['toid'];?>/<?php echo $all_messages_outbox['Message']['totype'];?>/<?php echo $all_messages_outbox['Message']['id'];?>/thismsg"><?php echo $all_messages_outbox['Messagecontent']['subject'] ?></a></td>
                    <td class="col-3">
                    ...
                    <td class="col-5">
			<span>
			    <a href="<?php echo BASE_URL;?>messages/conversation/<?php echo $all_messages_outbox['Message']['toid'];?>/<?php echo $all_messages_outbox['Message']['totype'];?>/<?php echo $all_messages_outbox['Message']['id'];?>/thismsg"><?php echo $this->Html->image('../frontend/images/icon35.jpg',array('alt'=>'')); ?></a>
			</span> 
