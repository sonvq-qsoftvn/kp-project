...
<script>
function comment_del(CommentId,ClinicId,PostId)
{
		
	$.ajax({
	    url : '<?php echo BASE_URL;?>clinics/delete_comment',
	    type: "POST",
	    data : {comment_id:CommentId,clinic_id:ClinicId,post_id:PostId},
	    success: function(data)
	    {
		//alert(data);
		var result = data.split('||');
		$("#pro_comm_shr_"+result[2]).html(result[1]);
...
<script>
	function saveAjaxcomment(postid)
	{
		$("#blank_comment_"+postid).hide();
		$("#load_comment_"+postid).show();
		$(".blank_comment").hide();
		$.ajax({
                        url: "<?php echo BASE_URL.'clinics/save_comment'; ?>",
                        type: "post",
                        data: $("#comment_form_"+postid).serialize(),
                        success: function(res) {
                            //alert(res);
			    var result = res.split('||');
			    //alert(result[0]); //Count of comment
			    //alert(result[1]); //Comment
			    //alert(result[2]); //save or edit
			   //alert(result[3]); //post id
			    
			    if (result!="" && result[2]=='save')
			    {
				//alert("hello");
				$(".comment_box").val('');
				$("#pro_comm_shr_"+result[3]).html(result[0]);
				if ($("#commentpop_"+postid).html()=="")
				{
					$("#commentpop_"+postid).html(result[1]);
				}
				else
				{
					$("#commentpop_"+postid).append(result[1]);
				}
				
			    }
			    else if(result!="" && result[2]=='edit')
			    {
				//alert("edit");
				//alert(result[1]);
				//alert(result[0]);
				...
				$("#comment_id_"+result[3]).val('');
			    }
			    else
			    {
				$("#blank_comment_"+postid).show();
				$("#blank_comment_"+postid).html(result[0]);
			    }
			$("#load_comment_"+postid).hide();
                        }
                    });
			...
			<div class="container pro">
				<div class="orchard-surgery">
					...
					<div class="orchard-buttons">
						...
						<a class="book_appointment" href="javascript:void(0)">Book Appointment</a>
						<?php
				...
				<div class="banner_slider banner_slider_wall">
					...
					?>
					<li>
					<?php echo $this->Html->image('../admin/company_logo/banner_logo/'.$eachbannerlogo['Companylogo']['company_logo'],array('alt'=>'')); ?></li>
					<?php
					}
					?>
		...
		<div class="seedoctor_orchardsurg">
			...
			<div class="seedoctor_orchardsurg_para">
				<?php
					...
					echo (isset($client_all_detail[0]['Clinic']['logo']))?$this->Html->image('../admin/uploads/thumb/'.$client_all_detail[0]['Clinic']['logo'],array('alt'=>'')):$this->Html->image('../frontend/images/no_clinic_logo.gif',array('alt'=>'','height'=>'225','weight'=>'200'));
				?>
				<br />
				<p>
				<?php if(isset($client_all_detail[0]['Clinic']['about']))
				{echo $client_all_detail[0]['Clinic']['about'];}
				else{echo "No information yet !";}?></p>
				<br />
				<p class="gray">
					
				<?php if($client_all_detail[0]['Clinic']['slogan']!='')
				{echo $client_all_detail[0]['Clinic']['slogan'];}
				else{echo "No slogan yet !";}?></p>
			</div>
			<div class="seedoctor_orchardsurg_items">
				...
				<div class="col-sm-4 col-md-4">
					<div class="see-orc">
					 ...
					 <?php  } else{?>
					 <div class="see-orc-left"> <?php echo $this->Html->image('../frontend/images/icon15.jpg',array('alt'=>'')); ?> </div><div class="see-orc-right"><h2>Opening Hour</h2><p>No information yet !</p></div>
					 <?php } ?>
					...
					?>
							<div class="see-orc">
								...
								<div class="see-orc-right">
									<h2>Waiting Time</h2>
									<p>
<?php echo ($client_all_detail[0]['Clinic']['waitingtime'])?'Between 0 to '.$client_all_detail[0]['Clinic']['waitingtime'].' minutes.':'Between 0 to 15 minutes.'; ?></p>
								</div>
							</div>
				<?php  	} ?>      
					<div class="see-orc">
				...
				<div class="col-sm-4 col-md-4">                
					<div class="see-orc">
						...
						<div class="see-orc-right">
							<h2>Contact us</h2>
							<?php 
								echo ($clinic_contact)?'<p>Phone: +'.$clinic_contact:'No information yet !</p>';
								echo ($clinic_contact)?'<br/><button type="" name="">Message Me</button><br/>':'';
							?>
					...
					<div class="see-orc">
						...
						<div class="see-orc-right">
							<h2>Co-Payment Eligibility</h2>
							<p>
								<?php
									if( !empty($current_eligibi))
									{ 
										$i=0;
	...
	<div class="wall_map">
		...
		<iframe  src="http://maps.google.co.in/maps?f=q&amp;source=s_q&amp;hl=en&amp;geocode=&amp;q=Blk+165+Bukit+Merah+Central+#01-3689,+Singapore+ &amp;aq=0&amp;oq=kol&amp;&amp;sspn=0.63665,1.352692&amp;ie=UTF8&amp;hq=&amp;hnear=Blk+165+Bukit+Merah+Central+#01-3689,+Singapore+ 150165&amp;&amp;spn=0.63658,1.352692&amp;t=m&amp;z=10&amp;output=embed"></iframe>      
	</div>
	...
	<div class="reviews">
		...
		<div class="inner-gapbox-1">
			<div class="orchard-surgery new_padding">
				<div class="orchard-buttons">
			<a class="book_appointment" href="<?php echo BASE_URL;?>clinics/addwallpost/<?php echo $clinic_id;?>" >Start New Topic</a>
				</div>
		...
		<div class="pro_pic">
		<?php
		if((($wall_post[$i]['User']['profile_image'])!='') && (file_exists("frontend/uploads/profile_image/".$wall_post[$i]['User']['profile_image'])))
		{
		?>
		<img src="<?php echo $this->webroot; ?>frontend/uploads/profile_image/thumbimage/<?php echo $wall_post[$i]['User']['profile_image']; ?>"  >
		<?php
		}
		else
		{
...
<?php echo $this->Html->image('../frontend/uploads/wallpost/thumbimage/'. $wall_post[$i]['Wallpost']['attachment_image'],array('alt'=>'')); ?>
					...
					<?php } ?>
					<div class="pro_comm_shr" id="pro_comm_shr_<?php echo $wall_post[$i]['Wallpost']['id'];?>">
					...
					<?php //pr($comments[$wall_post[$i]['Wallpost']['id']]);?>
		...
		<?php
		if((($value['u']['profile_image'])!='') && (file_exists("frontend/uploads/profile_image/".$value['u']['profile_image'])))
		{
		?>
		<img src="<?php echo $this->webroot; ?>frontend/uploads/profile_image/thumbimage/<?php echo $value['u']['profile_image']; ?>"  >
		<?php
		}
		else
		{
...
<strong><?php echo  $value['u']['username'];?> : </strong><span id="pcomment_<?php echo $value['Comment']['id'];?>"><?php echo $value['Comment']['comment'];?></span>
		...
		<?php if($this->Session->read('reid_user_uid') == $value['Comment']['user_id']){?>
			...
			<a onclick="comment_del('<?php echo $value['Comment']['id'];?>','<?php echo $clinic_id;?>','<?php echo $wall_post[$i]['Wallpost']['id']?>')" class="del_small"></a>
		...
		<!--<div class=comment_msg" id="comment_msg_<?php //echo $value['Comment']['id'];?>"></div>-->
					...
					<?php }?>
		<div id="commentpop_<?php echo $wall_post[$i]['Wallpost']['id']?>"></div>
		
		
		<span id="load_comment_<?php echo $wall_post[$i]['Wallpost']['id'];?>" style="color: red;display: none;"><?php echo $this->Html->image('../frontend/images/comment_loader.gif',array('alt'=>'')); ?></span> <!----- Loader image ----->			
		<div class="pro_new_comment">
		<div class="pro_comment_pic">
		<?php
		if((($logged_user['User']['profile_image'])!='') && (file_exists("frontend/uploads/profile_image/".$logged_user['User']['profile_image'])))
		{
		?>
		<img src="<?php echo $this->webroot; ?>frontend/uploads/profile_image/thumbimage/<?php echo $logged_user['User']['profile_image']; ?>"  >
		<?php
		}
		else
		{
		...
		<div class="pro_comment_text">
		<?php echo $this->Form->create('Comment',$settings=array( 'url' => array('controller' => 'clinics', 'action' => 'save_comment'),'class'=>'form-horizontal','id'=>'comment_form_'.$wall_post[$i]['Wallpost']['id'],'name'=>'comment_form')); ?>
		
	<textarea rows="5" cols="5" name="comment" placeholder="Write a comment..." id="comment_text_<?php echo $wall_post[$i]['Wallpost']['id'];?>" class="comment_box"></textarea>
		...
		<input type="button" name="save" value="Save" class="save" onclick="saveAjaxcomment('<?php echo $wall_post[$i]['Wallpost']['id']; ?>');"/>
		<?php echo $this->Form->end(); ?>
		
		<!--<a href="#"><?php //echo $this->Html->image('../frontend/images/icon27.jpg',array('alt'=>'')); ?></a>-->
		</div>
		<span id="blank_comment_<?php echo $wall_post[$i]['Wallpost']['id']; ?>" class="blank_comment" ></span>
		</div>
	    ...
	    <div class="getin_touch_title">
		 ...
		 <div class="getin_touch_items">
		    <div class="col-md-6 col-sm-6 col-xs-12">
                ...
                <?php 
				//echo $client_all_detail[0]['Clinic']['tags'];
				if($client_all_detail[0]['Clinic']['tags'])
				{
					$all_tags=explode(",",$client_all_detail[0]['Clinic']['tags']);
					foreach($all_tags as $each_tag)
					{
						if (strlen(trim($each_tag)) > 0) //if the string contains only Space. [Checking]
						echo "<li><a href='#'>".ucwords(trim($each_tag));"</a></li>";
					}
				}
					?>
...
<script>
	jQuery(document).ready(function(e) {
       jQuery('.bxslider').bxSlider({
			...
			/*if($('ul.bxslider li').hasClass('bx-clone')){
				$('.bx-wrapper .bx-pager .bx-pager-item:first-child').css({'display':'none'});
				$('.bx-wrapper .bx-pager .bx-pager-item:last-child').css({'display':'none'});	
			}*/
    });
