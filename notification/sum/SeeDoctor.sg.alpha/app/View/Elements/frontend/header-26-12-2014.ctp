...
<head>
	...
	if(!empty($like_notification))
	{
		foreach($like_notification as $likeValNoti)
		{
		?>
		<p><a href="<?php echo BASE_URL.'clinics/clincwall/'.$likeValNoti['Clinic']['id']; ?>"><?php echo $likeValNoti['User']['username'];?> like your <?php  echo $likeValNoti['Clinic']['name'];?> </a></p>
		<?php
		}
	}
	?>
	<?php
	if(!empty($clinic_approved_notification))
	{
		foreach($clinic_approved_notification as $clinicApprovedVal)
		{
		?>
	...
	{
		foreach($post_notification as $postnotiVal)
		{
		?>
	...
	if(!empty($reply_post_notification))
	{
		foreach($reply_post_notification as $replypostnotiVal)
		{
		?>
	...
	{
		foreach($message_notification as $messagenotiVal)
		{
			/*user type checking*/
		...
		<p><a href="<?php echo BASE_URL.'messages/conversation/'.$messagenotiVal['Messagecontent']['fromid'].'/'.$url_user.'/'.$messagenotiVal['Message']['id'];?>"> You have received a private message from <?php echo $allUser.' '.$messagenotiVal['Messagecontent']['subject'].' on '.date("d-m-Y h:i:s A",strtotime($messagenotiVal['Messagecontent']['datesent']));?> </a></p>
		<?php
		}
