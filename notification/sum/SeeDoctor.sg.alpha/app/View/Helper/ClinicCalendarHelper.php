<?php
     ...
     **/
     class ClinicCalendarHelper extends Helper
     {  
          /********************* PROPERTY ********************/
          private $Openinghour;
     ...
     private function _showDay($cellNumber, $day_avilable)
     {
          ...
          if(in_array($date_name, $day_avilable))
          {
               if($this->currentDate >= date('Y-m-d'))
               {
                    ...
                    {
                         $booked_slots = 0; $exception_chks = 0;
                         ...
                         $this->Appointment->unbindModel(
                                   array('belongsTo' => array('User', 'Oh', 'Clinic'))
                         );

                         
                         $booked_slots = $this->Appointment->find('count',array('conditions'=> array('Appointment.clinic_id' => $this->clinic_id, 'Appointment.slotid' => $clinic_opening_slot['Openinghour']['id'], 'Appointment.date' => $this->currentDate)));
                         
                         if($booked_slots > 0){ 
                              $total_slots = $total_slots - $booked_slots;
                         }
                         
                         $exception_chks = $this->Clinicexception->find('count', array('conditions'=>'clinicid="'.$this->clinic_id.'" AND exceptiondate = "'.$this->currentDate.'" AND cancelledslotid = "'.$clinic_opening_slot['Openinghour']['id'].'"'));
                         
                         if($exception_chks > 0){
                              $total_slots = $total_slots - $exception_chks;
                         }
     ...
     private function _createNavi(){
          ...
          return '<div class="month_title">'.
                    ...
                    '<div class="monthly_appointment_page">
                         <form name="cal_navigation" id="cal_navigation" action="'.BASE_URL.'appointments/clinic_appointment_details/id:'.$this->clinic_id.'/cal:month" method="post">
