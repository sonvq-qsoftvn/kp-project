...
function getCartDetails($admin_id,$unique)
{
	if($admin_id != ''){
	...
	{
		$sql="SELECT * FROM `".$this->prefix()."cart` where `unique_id` = '".$unique."' group by `event_id`";
	}
	$this->query($sql);
}
...
function getTicket($admin_id,$unique)
{
	if($admin_id != ''){
		$sql = "SELECT * FROM ".$this->prefix()."cart C LEFT join ".$this->prefix()."final_tickets T on (C.ticket_id = T.ticket_id)  WHERE C.user_id = ".$admin_id;
	}
	else{
		$sql = "SELECT * FROM ".$this->prefix()."cart C LEFT join ".$this->prefix()."final_tickets T on (C.ticket_id = T.ticket_id)  WHERE C.unique_id = ".$unique;
	}
	//echo $sql;
	$this->query($sql);
}
...
function totalAmount($admin_id,$type,$unique)
{
	if($admin_id != ''){
		if($type == 'mx'){
	...
	{
		if($type == 'mx'){
			$sql = "SELECT SUM(mx_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE unique_id = ".$unique;
			$this->query($sql);
		}
		elseif($type == 'us'){
			$sql = "SELECT SUM(us_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE unique_id = ".$unique;
		}
	}
