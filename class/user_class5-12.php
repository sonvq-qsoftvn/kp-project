...
{
	$sql="SELECT * FROM `".$this->prefix()."cart` where `user_id` = '".$admin_id."' OR `unique_id` = '".$unique."' group by `event_id`";
	$this->query($sql);
}
...
{
	$sql = "SELECT * FROM ".$this->prefix()."cart C LEFT join ".$this->prefix()."final_tickets T on (C.ticket_id = T.ticket_id)  WHERE C.user_id = '".$admin_id."' OR C.unique_id = ".$unique;
	$this->query($sql);
}
...
{
	if($type == 'mx'){
		$sql = "SELECT SUM(mx_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE user_id = '".$admin_id."' OR unique_id = ".$unique;
		$this->query($sql);
	}
	elseif($type == 'us'){
		$sql = "SELECT SUM(us_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE user_id = '".$admin_id."' OR unique_id = ".$unique;
		$this->query($sql);
	}
}
