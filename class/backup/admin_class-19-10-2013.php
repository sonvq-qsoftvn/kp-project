...
##############################ADDED BY DURBA ON 9th September, 2013#####################################################

function allPerformerList($limit,$performer_state = false,$performer_country = false,$performer_city = false,$venue = false,$show_pastevent = false)			
{
	$whereClause = '';
	if($performer_state)
		$whereClause = " AND event_performer_state = '".$performer_state."'";	
	
	if($performer_country)
		$whereClause .= " AND event_performer_county = '".$performer_country."'";	
	
	if($performer_city)
		$whereClause .= " AND  event_performer_city = '".$performer_city."'";	
	
	if($performer)
		$whereClause .= " AND  event_performer = '".$performer."'";	
	...
	$sql="SELECT A.performer_name_en,C.city_name,S.county_name,B.* FROM ".$this->prefix()."venue AS A, ".$this->prefix()."general_events AS B, ".$this->prefix()."city AS C, ".$this->prefix()."county S  WHERE A.performer_id = B.event_performer AND C.id = B.event_performer_city AND S.id = B.event_performer_county $whereClause ORDER BY S.county_name,C.city_name,A.venue_name,B.event_start_date_time ASC $limit";
