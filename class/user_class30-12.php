...
{
	if($admin_id != ''){
		$sql = "SELECT * FROM ".$this->prefix()."cart C join ".$this->prefix()."final_tickets T on (C.ticket_id = T.ticket_id)  WHERE C.user_id = ".$admin_id." and C.event_id = ".$event;
	}
	else{
...
function totalAmount($admin_id,$type,$unique,$event)
{
	if($admin_id != ''){
		if($type == 'mx'){
			$sql = "SELECT SUM(mx_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE user_id = ".$admin_id." and event_id = ".$event;
			$this->query($sql);
		}
		elseif($type == 'us'){
			$sql = "SELECT SUM(us_price*ticket) AS Amt FROM ".$this->prefix()."cart WHERE user_id = ".$admin_id." and event_id = ".$event;
		}
	}
	else
	{
