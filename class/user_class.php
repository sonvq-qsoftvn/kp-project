...
{
	$sql = "SELECT * FROM ".$this->prefix()."event_category  WHERE parent_category = '0' ORDER BY category_name ASC";
	$this->query($sql);
}

//=============== FETCH CATEGORY BY EVENT ID ===================//
function getCategoryByEventId($event_id) {
    $sql = "SELECT DISTINCT(cbe.category_id), ec.category_name, ec.category_name_sp FROM ".$this->prefix()."category_by_event as cbe LEFT JOIN " . $this->prefix() . "event_category as ec ON (cbe.category_id = ec.category_id) WHERE parent_category = '0' AND event_id = " . $event_id;
	$this->query($sql);
}

//=============== FETCH SCHEMA TYPE BY EVENT ID ===================//
function getSchemaTypeByEventId($event_id) {
    $sql = "SELECT DISTINCT(cbe.category_id), ec.category_name, ec.schema_eventType FROM ".$this->prefix()."category_by_event as cbe LEFT JOIN " . $this->prefix() . "event_category as ec ON (cbe.category_id = ec.category_id) WHERE parent_category = '0' AND event_id = " . $event_id;
	$this->query($sql);
}

//=============== FETCH SUB CATEGORY BY EVENT ID ===================//
function getSubCategoryByCategoryIdAndEventId($event_id, $category_id) {
...
function allEventSpotlightPicture()
{
	$sql = "SELECT * FROM `".$this->prefix()."general_events` WHERE now() <= event_start_date_time AND spotlight=1 AND event_photo != '' Order by event_id Desc";
	//echo $sql."<br />";
	return $this->query($sql);	
}
...
function edit_admin_details_new_on_payment($fname,$lname,$email,$phone,$country_id,$admin_id,$mobile_code)
{
	$sql="UPDATE ".$this->prefix()."admin SET fname = '".$fname."',
						lname = '".$lname."',
						email = '".$email."',
						mobile = '".$phone."',
						mobile_code = '".$mobile_code."',
						country_id = '".$country_id."' where admin_id = '".$admin_id."'";
	//echo $sql;exit;
	$this->query($sql);
}
...
function multi_event_datewise_index($event_id,$event_start_date_time,$next_date){
	...
	from (
        ...
        AND event_start_date_time > '".$event_start_date_time."'
		AND event_start_date_time < '".$next_date."'
		ORDER BY event_start_date_time
		)
	) a
...
#####################################CHANGE PASSWORD STARTS###################################################
	...
	{
		$sql="UPDATE ".$this->prefix()."admin SET  password = '".md5($password)."' WHERE admin_id= '".$userid."'";
		$this->query($sql);
	}		
...
function savepass($md5pass,$admin_id)
{
	$sql ="UPDATE ".$this->prefix()."admin SET  password = '".$md5pass."' WHERE admin_id = '".$admin_id."'";
	$this->query($sql); 
}
...
function update_register_user($fname,$lname,$email,$phone,$country_id,$country_code,$password,$account_type,$language,$aid)
{
	$sql="UPDATE ".$this->prefix()."admin SET fname = '".$fname."',
											  lname = '".$lname."',
											  email = '".$email."',
...
{
	$sql="UPDATE ".$this->prefix()."admin SET password ='".$password ."' where admin_id = '".$aid."'";
	//echo $sql;exit;
	$this->query($sql);
}
...
function bookingEventByUser($limit,$user_id){
	...
	{
		//$sql="SELECT C.`cart_id`,C.`transaction_id`,C.`event_id`,C.`multi_id`,C.`ticket`,C.`sdate`,C.`edate`,C.`start`,C.`end`,E.`admin_id`,E.`event_name_en`,E.`event_name_sp`,E.`event_short_desc_en`,E.`event_short_desc_sp`,E.`event_start_date_time`,E.`event_start_ampm`,E.`event_end_date_time`,E.`event_end_ampm`,E.`event_details_en`,E.`event_details_sp`,E.`event_photo`,V.`venue_id`,V.`venue_name`,V.`venue_name_sp`,V.`venue_short_add_sp`,V.`venue_short_add_en`,ct.`city_name`,ct.`city_name_sp` FROM ".$this->prefix()."cart C Inner join ".$this->prefix()."general_events E ON (E.event_id = C.event_id) Inner join ".$this->prefix()."venue V ON (E.event_venue = V.venue_id ) Inner join ".$this->prefix()."city ct ON (V.venue_city  = ct.id ) WHERE C.`user_id`='".$user_id."' ORDER BY C.`cart_id` DESC $limit ";
...
function allBookingEventByUser($limit,$user_id){
...
function allBookingEventByUserCount($user_id){
...
function addAdsSpotlightPicture($id)
{        
    $sql="SELECT * FROM ".$this->prefix()."ad ka join ".$this->prefix()."ad_content kac  ON ( ka.ad_id = kac.ad_id )  WHERE  kac.language_id='".$id."' AND ka.ad_size='full' AND ka.spotlight = 1 AND kac.ad_image_name != '' AND  curdate( ) >= ka.From_date AND curdate( ) < ka.duration ORDER BY ka.position_id,ad_size ASC "; 
    return $this->query($sql);        
}
...
function getAllMetaByPageId($page_id,$lang)
{
   $sql="SELECT * FROM ".$this->prefix()."meta_table where page_id='".$page_id."' and language_id='".$lang."'";	  
  return $this->query($sql);  
...
function eventTypeMasterByEventId($event_id)
{
    $sql="select * from  ".$this->prefix()."event_types as et left join ".$this->prefix()."master_event_types met on (et.event_master_type_id=met.event_master_type_id) where event_id='".$event_id."'";
    return $this->query($sql);	
}


function getStateCountyByEventID($event_id){

	$sql='SELECT st.state_name, co.county_name, ci.city_name FROM '.$this->prefix().'general_events eve Inner join '.$this->prefix().'state st ON (eve.event_venue_state = st.id ) Inner join '.$this->prefix().'county co on (eve.event_venue_county = co.id) Inner join '.$this->prefix().'city ci on (eve.event_venue_city = ci.id) WHERE eve.event_id="'.$event_id.'"  ';	
	return $this->query($sql);	
}
