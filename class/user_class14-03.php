...
function EventAll($limit = false,$tags,$category,$evn_city,$evn_venue,$start_date,$end_date,$key_word,$county_val,$event_categories){
	...
	if($start_date!="" && $end_date!="")
	{
		...
		$sql="select *,E.event_id as id from ".$this->prefix()."general_events E LEFT join ".$this->prefix()."venue V on (E.event_venue = V.venue_id ) $frmClause where E.`event_time`!= 'Weekly' AND E.event_start_date_time > '".$start_date."' AND E.event_start_date_time <= '".$end_date."' AND E.status = 'publish' $whereCluase order by E.`event_start_date_time` ASC";
	}
	else
	{
		$sql="select *,E.event_id as id from ".$this->prefix()."general_events E LEFT join ".$this->prefix()."venue V on (E.event_venue = V.venue_id ) $frmClause where E.`event_time`!= 'Weekly' AND now() <= E.event_start_date_time AND E.status = 'publish' $whereCluase order by E.`event_start_date_time` ASC";
	}
	//echo $sql;//exit;
	$this->query($sql);
}
...
{
	$sql="SELECT * FROM ".$this->prefix()."page  where path='".$page."' and page_id < '".$page_id."' order by page_id desc limit 0,1";
	$this->query($sql);
}
function blog_next($page,$page_id)
{
	$sql="SELECT * FROM ".$this->prefix()."page  where path='".$page."' and page_id > '".$page_id."' order by page_id asc limit 0,1";
	$this->query($sql);
}
//---------------------intro page end------------------------------
...
function eventDetails($event_county_cal,$event_cities_cal,$event_venues_cal,$event_categories_cal){
	...
	$sql="select *,E.event_id as evnt,V.venue_name as ven_nam,V.venue_name_sp as ven_nam_sp,C.city_name from ".$this->prefix()."general_events E Inner join ".$this->prefix()."venue V ON (E.event_venue = V.venue_id ) LEFT join ".$this->prefix()."city C ON (V.venue_city = C.id) $frmClause where event_time!= 'Weekly' and E.status = 'publish' $whereCluase Order by E.event_start_date_time ";
	//echo $sql; //exit;
...
function MultiEventAll($limit = false,$tags,$category,$evn_city,$evn_venue,$start_date,$end_date,$key_word,$county_val,$event_categories){
	...
	if($start_date!="" && $end_date!="")
	{
		...
		$sql="select *,E.event_id as id,ME.event_start_date_time as multi_start,ME.event_end_date_time as multi_end from ".$this->prefix()."general_events E Inner join ".$this->prefix()."final_multi_event ME on (E.event_id = ME.event_id ) LEFT join ".$this->prefix()."venue V on (ME.multi_venue = V.venue_id ) $frmClause where E.`identical_function`= 1 AND E.`recurring`= 0 AND ME.event_start_date_time > '".$start_date."' AND ME.event_start_date_time <= '".$end_date."' AND E.status = 'publish' $whereCluase order by ME.`event_start_date_time` ASC";
	}
	else
	{
		$sql="select *,E.event_id as id,ME.event_start_date_time as multi_start,ME.event_end_date_time as multi_end from ".$this->prefix()."general_events E Inner join ".$this->prefix()."final_multi_event ME on (E.event_id = ME.event_id ) LEFT join ".$this->prefix()."venue V on (ME.multi_venue = V.venue_id ) $frmClause where E.`identical_function`= 1 AND E.`recurring`= 0 AND now() <= ME.event_start_date_time AND E.status = 'publish' $whereCluase order by ME.`event_start_date_time` ASC";
	}
	//echo $sql;//exit;
	$this->query($sql);
}
...
function SubEventAll($limit = false,$tags,$category,$evn_city,$evn_venue,$start_date,$end_date,$key_word,$county_val,$event_categories){
	...
	if($start_date!="" && $end_date!="")
	{
		...
		$sql="select *,E.event_id as id,ME.event_start_date_time as multi_start,ME.event_end_date_time as multi_end,ME.event_id as sub_event_id,E.event_name_en as eve_name_en,E.event_name_sp as eve_name_sp from ".$this->prefix()."general_events E Inner join ".$this->prefix()."general_subevents ME on (E.event_id = ME.parent_id ) LEFT join ".$this->prefix()."venue V on (ME.event_venue = V.venue_id ) $frmClause where E.`sub_events`= 1 AND E.`recurring`= 0 AND ME.event_start_date_time > '".$start_date."' AND ME.event_start_date_time <= '".$end_date."' AND E.status = 'publish' $whereCluase order by ME.`event_start_date_time` ASC";
	}
	else
	{
		$sql="select *,E.event_id as id,ME.event_start_date_time as multi_start,ME.event_end_date_time as multi_end,ME.event_id as sub_event_id,E.event_name_en as eve_name_en,E.event_name_sp as eve_name_sp from ".$this->prefix()."general_events E Inner join ".$this->prefix()."general_subevents ME on (E.event_id = ME.parent_id ) LEFT join ".$this->prefix()."venue V on (ME.event_venue = V.venue_id ) $frmClause where E.`sub_events`= 1 AND E.`recurring`= 0 AND now() <= ME.event_start_date_time AND E.status = 'publish' $whereCluase order by ME.`event_start_date_time` ASC";
	}
	//echo $sql;//exit;
	$this->query($sql);
}
...
function savedEventByUser($limit,$user_id){
	...
	$sql="SELECT *,S.post_date as pdate,E.event_id as id FROM ".$this->prefix()."user_saved_events S Inner join ".$this->prefix()."general_events E ON (E.event_id = S.event_id) Inner join ".$this->prefix()."venue V ON (E.event_venue = V.venue_id ) where S.admin_id= '".$user_id."' $limit ";
	//echo $sql;
	$this->query($sql);
}
...
function subevent_datewise2($event_id,$event_start_date_time,$next_date){
	...
	$sqls="SELECT *, ME.event_start_date_time as multi_start_time, ME.event_end_date_time as multi_end_time,C.city_name as city_name1,ME.event_name_en as sub_eve_en,ME.event_name_sp as sub_eve_sp FROM ".$this->prefix()."general_events E Inner join ".$this->prefix()."general_subevents ME on (E.event_id = ME.parent_id) LEFT join ".$this->prefix()."venue V on (ME.event_venue = V.venue_id) LEFT join ".$this->prefix()."city C on (ME.event_venue_city = C.id ) where ME.parent_id='".$event_id."' and ME.event_start_date_time > '".$event_start_date_time."' AND ME.event_start_date_time < '".$next_date."' ORDER BY ME.event_end_date_time desc ";
	//echo $sqls."<br/><br/><br/>";
	return $this->query($sqls);
}
...
function add_to_cart($event_id,$multi_id,$ticket,$mx_price,$us_price,$unique,$tid,$payment,$date,$end_date,$start,$end,$general,$multi,$sub) {
...
function add_order($item_name,$item_number,$payment_status,$payment_amount,$payment_currency,$txn_id,$receiver_email,$payer_email,$event_id,$ticket,$payment,$ticket_id,$multi_id,$user_id,$order_no,$cart_id) {
