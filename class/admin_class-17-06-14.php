...
function add_gallery($admin_id,$media_url,$media_format,$set_privacy)
	{
		$sql="INSERT INTO ".$this->prefix()."media set admin_id='".$admin_id."',media_url='".$media_url."',media_format='".$media_format."',set_privacy='".$set_privacy."'";
		echo $sql;
		$rs=$this->query($sql);
		return mysql_insert_id();
	}
...
function allGalleryByID($event_id,$limit){
	...
	$sql="SELECT *,M.media_id as m_id FROM ".$this->prefix()."media M Inner join ".$this->prefix()."media_language ML ON (M.media_id = ML.media_id ) Inner join ".$this->prefix()."event_gallery EG on (EG.media_id = M.media_id)  WHERE EG.event_id=".$event_id." Order By M.media_id DESC $limit";
	//echo $sql;
	return $this->query($sql);
}
...
function allGalleryByID_count($event_id){
	...
	$sql="SELECT * FROM ".$this->prefix()."media M Inner join ".$this->prefix()."media_language ML ON (M.media_id = ML.media_id ) Inner join ".$this->prefix()."event_gallery EG on (EG.media_id = M.media_id)  WHERE EG.event_id=".$event_id." Order By M.media_id DESC";
	//echo $sql;
	return $this->query($sql);
}
...
function allGalleryNotInEvent($event_id,$admin_id){
  ...
  $sql="SELECT *,km.media_id as m_id FROM ".$this->prefix()."media as km, ".$this->prefix()."media_language as kml  WHERE km.media_id=kml.media_id and km.media_id IN (SELECT media_id FROM ".$this->prefix()."event_gallery WHERE event_id!='".$event_id."') and (km.set_privacy='0' OR (km.admin_id='".$admin_id."' AND km.set_privacy='1')) order by km.media_id DESC";
	//echo $sql;
	return $this->query($sql);
}
