...
function update_transaction_by_id($tid,$fee_incl_amount,$fee_non_incl_amount)
{
	$sql="UPDATE ".$this->prefix()."transaction SET fee_incl_amount = '".$fee_incl_amount."', fee_non_incl_amount = '".$fee_non_incl_amount."' where id = '".$tid."'";
	//echo $sql;
	//exit;
	$this->query($sql);
}
...
$sql="SELECT * FROM ".$this->prefix()."ad ka join ".$this->prefix()."ad_content kac  ON ( ka.ad_id = kac.ad_id )  WHERE  kac.language_id='".$id."' AND (ka.ad_size='banner' ||  ka.ad_size='full') AND  curdate( ) >= ka.From_date
AND curdate( ) < ka.duration ORDER BY ka.position_id ASC "; 
