...
{$lang="es";$org_lang = 'es';}
	...
	$objformeta->getAllMeta('home',$lang);
				   ...
				   if($_SESSION['langSessId']=='eng'){ 
					...
					if($objEventcal_rec->f('event_time') == "Weekly")
					{
						...
						if($allDate!=""){
							...
							// =========== SET URL =================================================	
								if($org_lang=="es")
								$event_url = $obj_base_path->base_path().'/evento/'.$objEventcal_rec->f('evnt').'/'.$org_lang;
								else
								$event_url = $obj_base_path->base_path().'/event/'.$objEventcal_rec->f('evnt').'/'.$org_lang;
							// =========== SET URL =================================================	

							?>
							{
								title: '<?php echo $lang.$eve_name.'\n'.$city_name.'\n'.$eve_venue.' \n'.date('g:i A',strtotime($starttime)); ?>',
				   ...
				   if($_SESSION['langSessId']=='eng'){ $eve_venue =  substr($objEventcal->f('ven_nam'),0,30); } else { $eve_venue = substr($objEventcal->f('ven_nam_sp'),0,30); };
					...
					else if($objEventcal->f('event_time') == "Monthly")
					{
						...
						if($allDateMonthly!=""){
							...
							// =========== SET URL =================================================	
								if($org_lang=="es")
								$event_url_cal = $obj_base_path->base_path().'/evento/'.$objEventcal->f('evnt').'/'.$org_lang;
								else
								$event_url_cal = $obj_base_path->base_path().'/event/'.$objEventcal->f('evnt').'/'.$org_lang;
							// =========== SET URL =================================================	
