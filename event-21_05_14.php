...
if(isset($_POST['action']) && $_POST['action'] == 'cart')	
{
	...
	echo $_POST['frm_us_price'.$i]."<br />";*/
		if($_POST['frm_ticket'.$i] != ''){
			$cid[] = $obj_cart->add_to_cart($_POST['frm_event_id'],$_POST['frm_multi_id'],$_POST['frm_ticket'.$i],$_POST['frm_mx_price'.$i],$_POST['frm_us_price'.$i],$unique,$_POST['frm_us_tid'.$i],$_POST['frm_payment'],$_POST['frm_date'],$_POST['frm_end_date'],$_POST['frm_start'],$_POST['frm_end'],$_POST['general'],$_POST['multi'],$_POST['sub']);
		}
	}
...
<meta property="og:description" content="<?php if($_SESSION['langSessId']=='eng') { echo date("l, j F, Y, g:i a",strtotime($objEvent->f('event_start_date_time'))).", ".$obj_venue->f('venue_name').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_en'); } else { echo date("l, j F, Y, g:i a",strtotime($objEvent->f('event_start_date_time'))).", ".$obj_venue->f('venue_name_sp').", ".$obj_venue->f('city').", ".$obj_venue->f('st_name').", ".$objEvent->f('event_short_desc_sp');}?>" />
...
?> 
<title><?php if($_SESSION['langSessId']=='eng') { echo stripslashes($objEvent->f('event_name_en')); } else { echo stripslashes($objEvent->f('event_name_sp'));}?></title>
...
<!--                	<span style="float: right; margin: 0 auto; padding: 0; width: 160px;">
	...
	<?php
			...
			?>
		       <div class="timetxt">
                        <?php echo strftime("%a",strtotime($obj_cur_eve_dt->f('multi_start_time')))." ".strftime("%b",strtotime($obj_cur_eve_dt->f('multi_start_time')))." ".strftime("%d",strtotime($obj_cur_eve_dt->f('multi_start_time'))).", ".strftime("%Y",strtotime($obj_cur_eve_dt->f('multi_start_time')));?> - <?php echo strftime('%l:%M%p',strtotime($obj_cur_eve_dt->f('multi_start_time'))); ?> to <?php echo strftime('%l:%M%p',strtotime($obj_cur_eve_dt->f('multi_end_time'))); ?>
                       </div>
			<?php }?>
                       <?php
								}
			...
			<?php if($_SESSION['langSessId']=='eng'){?>
		       ...
		       ?>
		       <div class="timetxt">
                        <?php echo strftime("%a",strtotime($event_date))." ".strftime("%b",strtotime($event_date))." ".strftime("%d",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date));?> - <?php echo strftime('%l:%M%p',strtotime($event_time)); ?> to <?php echo strftime('%l:%M%p',strtotime($event_time_end)); ?>
                       </div>
		       <?php }?>
                        <?php
							}
		       ...
		       ?>
		       <div class="timetxt">
                        <?php echo strftime("%a",strtotime($event_date))." ".strftime("%b",strtotime($event_date))." ".strftime("%d",strtotime($event_date)).", ".strftime("%Y",strtotime($event_date));?> - <?php echo strftime('%l:%M%p',strtotime($event_time));
						
						if($event_date != $event_date_end) 
						{ 
							echo " <br /> To ";
							echo strftime("%a",strtotime($event_date_end))." ".strftime("%b",strtotime($event_date_end))." ".strftime("%d",strtotime($event_date_end)).", ".strftime("%Y",strtotime($event_date_end))." - ";?> <?php echo strftime('%l:%M%p',strtotime($event_time_end)); 
						} 
						else 
						{ 
						?>
