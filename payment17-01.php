...
if( isset($_POST['hid_sign']) )
{
	...
	else
	{
		...
		$_SESSION['err'] = "login inválido. Por favor, inténtelo de nuevo.";
...
$user_id = $obj_adduser->register_user($fname,$lname,$email,$phone,$country_id,$country_code,$rem_password,$password,$account_type,$language,$province,$county,$city,$address,$postal_code);
...
<div id="maindiv">	
	...
	<div class="body_bg">    	
    	...
    	<div class="container">
            ...
            <div style="text-align:center;"><?php if($_SESSION['err'] != ''){ echo $_SESSION['err'];}?></div>
            <div class="clear"></div>
				...
				<div class="view_box7">
					...
					$date = $obj_cart_details->f('edate');
					?>
					<p><?php echo $date." ".$start;?> - <?php echo $date." ".$end;?></p>
...
<?php if($_SESSION['langSessId']=='eng') {?>  MX Pesos <?php }elseif($_SESSION['langSessId']=='spn'){?> Pesos MX <?php }?> <!--<a href="<?php //echo $obj_base_path->base_path()."/payment.php?act=mx"; ?>"><input type="radio" name="cur" id="" value="" <?php //if($_SESSION['pay'] == 'mx'){ echo "checked";}?> /></a>-->
 ...
 <?php if($_SESSION['langSessId']=='eng') {?>  US$ <?php }elseif($_SESSION['langSessId']=='spn'){?>EE.UU. $<?php }?> <!--<a href="<?php //echo $obj_base_path->base_path()."/payment.php?act=us"; ?>"><input type="radio" name="cur" id="" value="" <?php //if($_SESSION['pay'] == 'us'){ echo "checked";}?> /></a> -->
					...
					{
					?>
<?php if($_SESSION['langSessId']=='eng') {?>  MX Pesos <?php }elseif($_SESSION['langSessId']=='spn'){?> Pesos MX <?php }?> <a href="<?php echo $obj_base_path->base_path()."/payment.php?event_id=".$e_id."&act=mx"; ?>"><input type="radio" name="cur" id="" value="" <?php if($_SESSION['pay'] == 'mx'){ echo "checked";}?> /></a>

<?php if($_SESSION['langSessId']=='eng') {?>  US$ <?php }elseif($_SESSION['langSessId']=='spn'){?>EE.UU. $<?php }?> <a href="<?php echo $obj_base_path->base_path()."/payment.php?event_id=".$e_id."&act=us"; ?>"><input type="radio" name="cur" id="" value="" <?php if($_SESSION['pay'] == 'us'){ echo "checked";}?> /></a> 
              ...
              <div class="blue_bg" style="margin: 0 auto;"><?php if($_SESSION['langSessId']=='eng') {?>Buyers Information<?php }elseif($_SESSION['langSessId']=='spn'){?>Información del comprador<?php }?></div> 
			  <div class="clear"></div>
		...
		<?php if($_SESSION['langSessId']=='eng'){
		  echo "Or let’s check your email address Check";
		  }elseif($_SESSION['langSessId']=='spn'){
		    echo "¿O vamos a comprobar su dirección de correo electrónico Compruebe";
                ...
                <td><a href="javascript:void(0);" onClick="check_user();" class="checkbtn">
		<?php if($_SESSION['langSessId']=='eng'){
		  echo "Check";
		  }elseif($_SESSION['langSessId']=='spn'){
		    echo "Cheque";
           ...
           <table width="100%" align="center" border="0" cellpadding="4" cellspacing="4" style="border-collapse:separate;">
              ...
              <tr>
                ...
                <td>
                  ...
                  <input onClick="display();" type="text" name="phone" id="phone" class="textbg_grey" value="<?php echo $obj->f('mobile')?>" style="width: 190px;" onKeyUp="checkPhone()" />
		    ...
		    ?>
              ...
              <div id="display" style="display: none;"><input type="submit" value="Update" class="btn1_sudip" /></div>
            </table>
		   <?php
           }
		   ?>
	...
	<?php }?>
		 ...
		 <li><a href="javascript:void(0);" >Credit/Debit Card</a><br/><img src='<?php echo $obj_base_path->base_path(); ?>/images/logo5.gif' width="35" height="19" border="0"/><br/>
		   <input name="radiobutton" id="standard" type="radio" value="radiobutton" onClick="pay_type('pro');"/></li>
		 <li><a href="#">Paypal</a><br/><img src='<?php echo $obj_base_path->base_path(); ?>/images/logo6.gif' width="35" height="19" border="0"/><br/><input name="radiobutton" id="pro" type="radio" value="radiobutton" onClick="pay_type('standard');"/></li>
		 <li><a href="#">Bank Transfer</a><br/><img src='<?php echo $obj_base_path->base_path(); ?>/images/logo7.gif' width="40" height="21" border="0"/><br/><input name="radiobutton" id="radiobutton" type="radio" value="radiobutton"/></li>
		 <?php if($_SESSION['pay'] == 'mx'){?>
         <li style="border: 0;"><a href="#">Cash</a><br/><img src='<?php echo $obj_base_path->base_path(); ?>/images/logo8.gif' width="44" height="21" border="0"/><br/><input name="radiobutton" id="radiobutton" type="radio" value="radiobutton"/></li>
         <?php }?>
		 </ul>
		 </div>
		 </div>
	...
	<?php }elseif($_SESSION['langSessId']=='spn'){?>
	Al hacer clic en el botón "Enviar pedido", estás aceptando la Política de Compra y Política de Privacidad KPasapp.com. Todos los pedidos están sujetos a la aprobación de los pagos y a la verificación de direcciones de facturación. Por favor, póngase en contacto con atención al cliente si tiene alguna pregunta acerca de su pedido.
	<?php }?>
	...
	<input type="hidden" name="type" value="checkout" />
	 <div align="center"><input type="button" onclick="new_checkout();" value="Submit Order" class="btn1_sudip" /></div>
          </form>                 
          </div>
